...
QStringList corelib::getCdromDevices(void) const{
   ...
   case 3:
        ...
        QString corelib::createQ4WineString(QString prefixName, QString dirName, QString iconName){
            QHash<QString, QString> icon = db_icon.getByName(prefixName, dirName, iconName);
            QHash<QString,QString> prefix = db_prefix.getByName(prefixName);

            QString run_string = QString("%1/bin/q4wine-helper").arg(APP_PREF);

            run_string.append(" --prefix ");
            run_string.append(prefixName);

            if (icon["nice"]=="0"){
                run_string.append(" --nice ");
                run_string.append(icon["nice"]);
            }

            if (!icon["desktop"].isEmpty()){
                run_string.append(" --desktop ");
                run_string.append(icon["desktop"]);
            }

            if (!icon["override"].isEmpty()){
                run_string.append(" --override \"");
                run_string.append(icon["override"]);
                run_string.append("\"");
            }

            if (icon["useconsole"]=="1"){
                run_string.append(" --console ");
                run_string.append(icon["useconsole"]);
            }

            if (!icon["winedebug"].isEmpty()){
                run_string.append(" --wine-debug ");
                run_string.append(icon["winedebug"]);
            }

            if (!icon["display"].isEmpty()){
                run_string.append(" --display ");
                run_string.append(icon["display"]);
            }

            if (!icon["cmdargs"].isEmpty()){
                run_string.append(" --program-args ");
                run_string.append(icon["cmdargs"]);
            }

            if (!icon["exec"].isEmpty()){
                run_string.append(" --program-bin ");
                run_string.append(icon["exec"]);
            }

            QString wrkdir = icon["wrkdir"];

            if (wrkdir.isEmpty())
                wrkdir = QDir::homePath();

            run_string.append(" --wrkdir ");
            run_string.append(wrkdir);
