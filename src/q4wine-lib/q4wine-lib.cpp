...
corelib::corelib(const bool GUI_MODE)
    : m_GUI_MODE(
          GUI_MODE)  // Setting gui mode, if false - cli mode else gui mode
{}

corelib *createCoreLib(const bool GUI_MODE) { return new corelib(GUI_MODE); }
...
const QString corelib::getGenericConfigLocation(const QStringList &path) {
  return formatLocation(QStringList()
...
QList<QStringList> corelib::getWineProcessList(const QString &prefix_name) {
  ...
  if (!prefix_name.isEmpty()) prefix_path = db_prefix.getPath(prefix_name);

  QString name, procstat, path, prefix, env_arg, nice, message;

#ifdef _OS_LINUX_
  message =
      "<p>Process is unable to access /proc file system.</p><p>Access is "
      "necessary for displaying Wine process information.</p><p>You need to "
      "set CONFIG_PROC_FS=y option on Linux kernel config file and mount proc "
      "file system by running: mount -t proc none /proc</p>";
#endif
  ...
  message =
      "<p>Process is unable to access /proc file system.</p><p>Access is "
      "necessary for displaying Wine process information.</p><p>You need to "
      "set PSEUDOFS and PROCFS option on FreeBSD kernel config file and mount "
      "proc file system by running: mount -t procfs proc /proc</p>";
#endif
  ...
  for (int i = 0; i < list.size(); ++i) {
...
#ifdef DEBUG
      //        qDebug()<< "Target for: " << info.filePath() << " is: " <<
      //        fileName;
#endif
      if ((!fileName.contains("wine") && !fileName.contains(".exe")) ||
          fileName.contains(APP_SHORT_NAME)) {
        continue;
      }
    } else {
      continue;
    ...
    if (file_c.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      name = in.readLine().split(QChar('\0')).at(0);
#ifdef DEBUG
      ...
      if (file_s.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        if (!line.isNull()) {
          ...
          line.replace(QRegularExpression("\\(.*\\)"), "name");
  ...
  /* On FreeBSD:
   * This is new engine for getting process info from /proc directory and kmem
   * interface
   */
#ifdef _OS_FREEBSD_
  kvm_t *kd;
  ...
  if (!kd) {
    if (this->showError(
            QObject::tr(
                "<p>It seems that Q4Wine cannot run kvm_openfiles.</p>"),
            false) == QMessageBox::Ignore) {
      procline << "-1";
      proclist << procline;
      return proclist;
    }
  ...
  if (!kp) {
    if (this->showError(
            QObject::tr("<p>It seems that Q4Wine cannot run kvm_getprocs.</p>"),
            false) == QMessageBox::Ignore) {
      procline << "-1";
      proclist << procline;
      return proclist;
  ...
  for (i = 0; i < cntproc; i++) {
    ...
    if (cur_pids.indexOf(QString("%1").arg(ipid)) == -1) {
      ...
      if ((name.contains("wine") || name.contains(".exe")) &&
          !name.contains(APP_SHORT_NAME)) {
        ni = kp[i].ki_nice;
        ...
        if (envs) {
          ...
          prefix = QObject::tr(
              "Cannot read process info from /proc or API is changed.");
#ifdef DEBUG
...
QVariant corelib::getSetting(const QString &group, const QString &key,
                             const bool checkExist,
  ...
  if (checkExist == true)
    if (!QFileInfo(retVal.toString()).exists()) {
      this->showError(
          QObject::tr("<p>Error while loading application settings by key: "
                      "'%1'. File or path does not exist: \"%2\"</p><p>Please, "
                      "go to the %3 options dialog and set it.</p>")
              .arg(key)
              .arg(retVal.toString())
              .arg(APP_SHORT_NAME));
      retVal = QVariant();
...
void corelib::checkSettings() {
  ...
  if (this->getWhichOut(
              this->getSetting("system", "gui_sudo", false).toString(), false)
          .isEmpty()) {
    QStringList guisudo;
#ifdef _OS_DARWIN_
    guisudo << "osascript";
  ...
  if (this->getSetting("advanced", "prefixDefaultPath", false)
          .toString()
          .isEmpty()) {
    ...
    if (qtt.load(lang, l10nPath)) {
      ...
      if (qtt.load(lang, l10nPath)) {
        ...
        if (qtt.load("q4wine_en", l10nPath)) {
          ...
          qDebug() << "[EE] Cannot open default translation, not using "
                      "translation engine.";
        }
      }
    }
...
void corelib::getBuildFlags() {
  ...
  Qcout << qSetFieldWidth(25) << Qt::left << " CMAKE_INSTALL_PREFIX"
        << QString::fromUtf8(APP_PREF) << qSetFieldWidth(0) << Qt::endl
        << Qt::endl;
#ifdef RELEASE
  Qcout << qSetFieldWidth(25) << Qt::left << " RELEASE" << "ON"
        << qSetFieldWidth(0) << Qt::endl;
#else
  Qcout << qSetFieldWidth(25) << Qt::left << " RELEASE" << "OFF"
        << qSetFieldWidth(0) << Qt::endl;
#endif
#ifdef DEBUG
  Qcout << qSetFieldWidth(25) << Qt::left << " DEBUG" << "ON"
        << qSetFieldWidth(0) << Qt::endl;
#else
  Qcout << qSetFieldWidth(25) << Qt::left << " DEBUG" << "OFF"
        << qSetFieldWidth(0) << Qt::endl;
#endif
#ifdef WITH_ICOUTILS
  Qcout << qSetFieldWidth(25) << Qt::left << " WITH_ICOUTILS" << "ON"
        << qSetFieldWidth(0) << Qt::endl;
#else
  Qcout << qSetFieldWidth(25) << Qt::left << " WITH_ICOUTILS" << "OFF"
        << qSetFieldWidth(0) << Qt::endl;
#endif
#ifdef WITH_WINEAPPDB
  Qcout << qSetFieldWidth(25) << Qt::left << " WITH_WINEAPPDB " << "ON"
        << qSetFieldWidth(0) << Qt::endl;
#else
  Qcout << qSetFieldWidth(25) << Qt::left << " WITH_WINEAPPDB " << "OFF"
        << qSetFieldWidth(0) << Qt::endl;
#endif
#ifdef WITH_DBUS
  Qcout << qSetFieldWidth(25) << Qt::left << " WITH_DBUS" << "ON"
        << qSetFieldWidth(0) << Qt::endl;
#else
  Qcout << qSetFieldWidth(25) << Qt::left << " WITH_DBUS" << "OFF"
        << qSetFieldWidth(0) << Qt::endl;
#endif
  Qcout << Qt::endl;
}
...
QString corelib::getWhichOut(const QString &fileName, bool showErr) {
  ...
  if (fileName == "mdconfig") {
    if (!mdconfig.isEmpty()) return mdconfig;
  } else if (fuseiso == "fuseiso") {
    if (!fuseiso.isEmpty()) return fuseiso;
  } else if (fileName == "fusermount") {
    if (!fusermount.isEmpty()) return fusermount;
  } else if (fileName == "ln") {
    if (!ln.isEmpty()) return ln;
  } else if (fileName == "rm") {
    if (!rm.isEmpty()) return rm;
  } else if (fileName == "sh") {
  ...
  if (!string.isEmpty()) {
    ...
    if (showErr)
      this->showError(
          QObject::tr("Cannot find or execute the '%1' binary. Make sure that "
                      "this binary is available by search PATH variable and "
                      "see also INSTALL file for application depends.")
              .arg(fileName));
  }
...
QStringList corelib::getCdromDevices(void) const {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (fileInfo.fileName().contains(QRegularExpression("^cdrom")) or
        fileInfo.fileName().contains(QRegularExpression("^sr")) or
        fileInfo.fileName().contains(QRegularExpression("^dvd"))) {
#elif defined(_OS_FREEBSD_)
    if (fileInfo.fileName().contains(QRegularExpression("^cdrom")) or
        fileInfo.fileName().contains(QRegularExpression("^cd")) or
        fileInfo.fileName().contains(QRegularExpression("^acd")) or
        fileInfo.fileName().contains(QRegularExpression("^dvd"))) {
...
QChar corelib::getCdromWineDrive(QString prefix_path,
...
QStringList corelib::getWineDlls(QString prefix_lib_path) const {
  ...
  if (prefix_lib_path.isEmpty()) {
    prefix_lib_path =
        this->getSetting("wine", "WineLibs", false, "").toString();
  }
...
QString corelib::getMountedImages(QString cdrom_mount) const {
    ...
    while (in.readLineInto(&line)) {
      ...
      if (line.contains(cdrom_mount)) {
        ...
        if (image.contains("fuseiso") || image.contains("q4wine-mount")) {
          ...
          //  I knew it is a horrible dirty hack, but I cannot find any other
          //  way to get information from loop device. If some one knew how to
          //  fix it, feel free to mail solution to me :]
          if (!this->getSetting("system", "sudo").toString().isEmpty()) {
            arguments << "losetup" << image;
            QProcess myProcess;
            myProcess.start(this->getSetting("system", "sudo").toString(),
                            arguments);
            if (!myProcess.waitForFinished()) {
              ...
              return image.split("/").last().mid(
                  0, image.split("/").last().length() - 2);
            }
          }
        } else {
          return image;
  ...
  arguments << "-c"
            << QString("%1 | grep \"%2\"")
                   .arg(this->getSetting("system", "mount").toString())
                   .arg(cdrom_mount);
    ...
    if (!image.isEmpty()) {
      if (image.contains("md")) {
        ...
        if (!myProcess.waitForFinished()) {
          ...
          image = myProcess.readAll();
          image = image.split("/").last().mid(
              0, image.split("/").last().length() - 1);
        }
  ...
  QRegularExpression rx_image("^image-path      : (.*)$");
  QRegularExpression rx_mount("^/dev/disk.*(/.*)$");
  ...
  /* plist parse info
      ...
      for (int i = 0; i < lol.size(); i++) {
          QString point =
     lol.at(i).toMap()["system-entities"].toMap()["mount-point"].toString(); if
     (point == cdrom_mount){ image =
     lol.at(i).toMap()["image-path"].toString().split("
     ").first().split("/").last(); break;
          }
      }
      */
...
bool corelib::runIcon(const QString &prefix_name, const QString &dir_name,
                      const QString &icon_name, const QStringList &extra_args) {
  QHash<QString, QString> result =
      db_icon.getByName(prefix_name, dir_name, icon_name);
  ExecObject execObj;
  ...
  if (extra_args.isEmpty()) {
    ...
    execObj.cmdargs =
        QString("%1 %2").arg(result.value("cmdargs")).arg(extra_args.join(" "));
  }
  ...
  if (path.mid(0, 1) == "/") {
    if (!QFile(path).exists()) {
      if (m_GUI_MODE) {
        QMessageBox::warning(
            ...
            QObject::tr("Binary file \"%1\" does not exist.").arg(path));
      } else {
    ...
    if (u_path.isEmpty()) {
      if (m_GUI_MODE) {
        QMessageBox::warning(
            0, QObject::tr("Error"),
            QObject::tr("Cannot get unix path for \"%1\".").arg(path));
      } else {
      ...
      if (!QFile(u_path).exists()) {
        if (m_GUI_MODE) {
          QMessageBox::warning(
              0, QObject::tr("Error"),
              QObject::tr("Binary file \"%1\" does not exist.").arg(u_path));
        } else {
...
bool corelib::runWineBinary(const ExecObject &execObj,
                            const QString &prefix_name, const bool detach) {
#ifdef RELEASE
#ifdef _OS_DARWIN_
  QString binary = QString("%1/%2.app/Contents/MacOS/q4wine-helper")
                       .arg(QDir::currentPath(), APP_SHORT_NAME);
#else
  QString binary =
      QString("%1/bin/q4wine-helper").arg(QString::fromUtf8(APP_PREF));
#endif
#else
#ifdef _OS_DARWIN_
  QString binary =
      QString("%1/q4wine-helper/q4wine-helper.app/Contents/MacOS/q4wine-helper")
          .arg(QString::fromUtf8(APP_BUILD));
#else
  QString binary = QString("%1/q4wine-helper/q4wine-helper")
                       .arg(QString::fromUtf8(APP_BUILD));
#endif
#endif
  QStringList args;
  ...
  if (wrkdir.isEmpty()) wrkdir = QDir::currentPath();

  args.append("--wrkdir");
  args.append(wrkdir);

#ifdef DEBUG
  qDebug() << "[ii] corelib::runWineBinary: " << binary << args
           << " at: " << wrkdir;
#endif
  ...
  if (detach) {
    ...
    Process proc(args, binary, QDir::currentPath(),
                 QObject::tr("Running binary: \"%1\"").arg(execObj.execcmd),
                 QObject::tr("Running binary..."), false);
    return proc.exec();
  }
...
QString corelib::createDesktopFile(const QString &prefix_name,
                                   const QString &dir_name,
                                   const QString &icon_name,
                                   const bool is_menu) const {
  QHash<QString, QString> result =
      db_icon.getByName(prefix_name, dir_name, icon_name);
  ...
  // FIXME:
  QString base_icon =
      QString("%1/applications/").arg(this->getGenericDataLocation());

#ifdef _OS_DARWIN_
  QString embedded_icon_path =
      QString("%1/%2.app/Contents/Resources/icons/share/q4wine/icons/")
          .arg(APP_PREF, APP_NAME);
#else
  ...
  out << "Exec=" << QString::fromUtf8(APP_PREF) << "/bin/q4wine-cli -p \""
      << prefix_name << "\" ";
  if (!dir_name.isEmpty()) out << " -d \"" << dir_name << "\" ";
  ...
  if (icon_path.isEmpty()) {
    ...
    if (QFile(icon_path).exists()) {
      ...
      if (icon_name == "eject") {
        ...
        out << "Icon=" << embedded_icon_path << "winefile" << ".svg"
            << Qt::endl;
        ...
        out << "Icon=" << embedded_icon_path << "oic_winlogo" << ".svg"
            << Qt::endl;
        ...
        out << "Icon=" << embedded_icon_path << "trash_file" << ".svg"
            << Qt::endl;
  ...
  out << "StartupWMClass="
      << result.value("exec").split('/').last().split('\\').last() << Qt::endl;
...
bool corelib::deleteDesktopFile(const QString &prefix_name,
                                const QString &dir_name,
...
QString corelib::getEscapeString(const QString &string,
                                 const bool spaces) const {
  if (spaces) {
...
bool corelib::mountImage(const QString &image_name,
  ...
  if (mount_point.isEmpty()) {
    this->showError(
        QObject::tr("It seems that no mount point was set in prefix "
                    "options.<br>You might need to set it manually."));
    return false;
  }
  if (image_name.isEmpty()) return false;
  ...
  if ((image_name.contains("/") &&
       (!image_name.contains(".iso", Qt::CaseInsensitive)) &&
       (!image_name.contains(".nrg", Qt::CaseInsensitive)))) {
    mount_string =
        this->getSetting("quickmount", "mount_drive_string", false).toString();
#ifdef DEBUG
    qDebug() << "[ii] corelib::mountImage:FreeBSD drive mount base string: "
             << mount_string;
#endif
    mount_string.replace("%MOUNT_DRIVE%", this->getEscapeString(image_name));
  } else {
    mount_string =
        this->getSetting("quickmount", "mount_image_string", false).toString();
#ifdef DEBUG
    qDebug() << "[ii] corelib::mountImage:FreeBSD image mount base string: "
             << mount_string;
#endif

    if (!QFile(image_name).exists()) {
      mount_string.replace("%MOUNT_IMAGE%",
                           this->getEscapeString(db_image.getPath(image_name)));
    } else {
  ...
  mount_string.replace("%GUI_SUDO%",
                       getSetting("system", "gui_sudo").toString());
  ...
  if ((image_name.contains("/") &&
    ...
    mount_string =
        this->getSetting("quickmount", "mount_drive_string", false).toString();
#ifdef DEBUG
    qDebug() << "[ii] corelib::mountImage:Linux drive mount base string: "
             << mount_string;
#endif
    mount_string.replace("%MOUNT_DRIVE%", this->getEscapeString(image_name));
  } else {
    mount_string =
        this->getSetting("quickmount", "mount_image_string", false).toString();
#ifdef DEBUG
    qDebug() << "[ii] corelib::mountImage:Linux image mount base string: "
             << mount_string;
#endif
    if (!imageFile.exists()) {
...
#endif
  bool success = this->runProcess(
      args, QObject::tr("Mounting..."),
      QObject::tr("Mounting %1 into %2").arg(image_name).arg(mount_point));
  if (success) {
    // create the symlink to the iso so that wine recognises the mountpoint as a
    // CD-ROM
    QString prefixPath = db_prefix.getPath(prefix_name);
    QChar winDrive = db_prefix.getMountPointWindrive(prefix_name);
    if (winDrive.isNull()) {
      winDrive =
          this->getCdromWineDrive(db_prefix.getPath(prefix_name), mount_point);
      if (winDrive.isNull()) {
#ifdef DEBUG
        qDebug()
            << "[ii] Prefix '" << prefix_name
            << "' does not have a Windows drive set for the mount operation";
#endif
        return success;  // do not create the link, return true
      }
    }
    // drive letter plus two colons links to the actual physical device (in this
    // case the image)
    ...
    QFile mountPointLink(prefixPath + "/dosdevices/" + winDrive.toLower() +
                         ":");
    if (mountPointLink.exists() &&
        mountPointLink.symLinkTarget() != mount_point) {
...
bool corelib::umountImage(const QString &prefix_name) {
...
#ifdef DEBUG
    qDebug()
        << "[ii] corelib::umountImage: no mounted images found in mount point: "
        << mount_point;
#endif
    return true;
  }
  ...
  QString mount_string;
  mount_string =
      this->getSetting("quickmount", "umount_string", false).toString();
#ifdef DEBUG
  ...
  mount_string.replace("%GUI_SUDO%",
                       getSetting("system", "gui_sudo").toString());
  mount_string.replace("%SUDO%", getSetting("system", "sudo").toString());
  mount_string.replace("%UMOUNT_BIN%",
                       getSetting("system", "umount").toString());
  mount_string.replace("%MOUNT_POINT%", this->getEscapeString(mount_point));
  ...
  return this->runProcess(args, QObject::tr("Umounting..."),
                          QObject::tr("Umounting point: %1").arg(mount_point));
}

bool corelib::runProcess(const QStringList &args, const QString &caption,
  ...
  if (m_GUI_MODE) {
    Process proc(args, this->getSetting("system", "sh").toString(),
                 QDir::homePath(), message, caption, false);
    return (proc.exec());
  } else {
    return (
        this->runProcess(this->getSetting("system", "sh").toString(), args));
  }
  return true;
}
...
bool corelib::runProcess(const QString &exec, const QStringList &args,
                         QString dir, const bool showLog) {
  ...
  if (!myProcess.waitForFinished()) return false;
  ...
  if (showLog && (exitcode != 0 || exitStatus == QProcess::CrashExit)) {
    ...
    // FIMXE: Does anyone use non utf8?
    // QTextCodec *codec = QTextCodec::codecForLocale();
    QString string = myProcess.readAllStandardError();

    if (!string.isEmpty()) {
      showError(QObject::tr("It seems that the process crashed. STDERR log: %1")
                    .arg(string));
    }
    return false;
  }
...
int corelib::showError(const QString &message, const bool info) const {
  if (m_GUI_MODE) {
    if (info) {
      ...
      return QMessageBox::warning(0, QObject::tr("Error"), message,
...
bool corelib::killWineServer(const QString &prefix_path, const QString &pid) {
  if (!pid.isEmpty()) {
    ...
    if (!dir.exists()) {
      message =
          "<p>Process is unable to access /proc file system.</p><p>Access is "
          "necessary for displaying Wine process information.</p><p>You need "
          "to set CONFIG_PROC_FS=y option on Linux kernel config file and "
          "mount proc file system by running: mount -t proc none /proc</p>";
      if (this->showError(message, false) == QMessageBox::Ignore) {
        return false;
      }
    }
...
#ifdef _OS_FREEBSD_
      string =
          "%GUI_SUDO% \"%MOUNT_BIN% -t cd9660 %MOUNT_DRIVE% %MOUNT_POINT%\"";
#endif
      break;
    case 2:
...
void corelib::openConsole(const QString &path, const QString &prefix_name) {
  ...
  // Start shell
  {
    ...
    sh_args << QString("WINEPREFIX='%1'")
                   .arg(getStrictEscapeString(prefix_info.value("path")));
    if (!prefix_info.value("server").isEmpty()) {
      sh_args << QString("WINE='%1'")
                     .arg(getStrictEscapeString(prefix_info.value("bin")));
      if (!prefix_info.value("libs").isEmpty())
        sh_args << QString("WINEDLLPATH='%1'")
                       .arg(getStrictEscapeString(prefix_info.value("libs")));
      sh_args << QString("WINELOADER='%1'")
                     .arg(getStrictEscapeString(prefix_info.value("loader")));
      sh_args << QString("WINESERVER='%1'")
                     .arg(getStrictEscapeString(prefix_info.value("server")));
    } else {
      ...
      if (vers.load()) {
        ...
        sh_args
            << QString("WINE='%1'").arg(getStrictEscapeString(vers.wine_exec_));
        if (!prefixDllPath.isEmpty())
          sh_args << QString("WINEDLLPATH='%1'")
                         .arg(getStrictEscapeString(prefixDllPath));
        sh_args << QString("WINELOADER='%1'")
                       .arg(getStrictEscapeString(vers.wine_loader_));
        sh_args << QString("WINESERVER='%1'")
                       .arg(getStrictEscapeString(vers.wine_server_));
      }
    }
    ...
    sh_args << "/bin/sh"
            << "-c"
            << QString(
                   "\"echo \'\\n\\tq4wine: wine environment variables are set "
                   "to \\\"%2\\\" prefix settings.\\n\' && %3 \"")
                   .arg(prefix_name)
                   .arg(shell);
...
#ifdef _OS_LINUX_
      string =
          "%GUI_SUDO% \"%MOUNT_BIN% %MOUNT_OPTIONS% %MOUNT_IMAGE% "
          "%MOUNT_POINT%\"";
#endif
#ifdef _OS_FREEBSD_
      string = "";
...
bool corelib::reniceProcess(const int pid, const int priority) {
...
#ifdef _OS_DARWIN_
  args << "-e";
  args << QString("do shell script \"%1 %2 %3\" with administrator privileges")
              .arg(this->getSetting("system", "renice").toString())
              .arg(priority)
              .arg(pid);
#else
  ...
  if (!sudobin.contains(QRegularExpression("/(sudo|pkexec)$"))) {
    QString arg = args.join(" ");
    args.clear();
    args.append(arg);
  }
#endif
  if (this->runProcess(this->getSetting("system", "gui_sudo").toString(), args,
                       QDir::homePath(), false)) {
    return true;
  } else {
    return false;
...
void corelib::createPrefixDBStructure(const QString &prefixName) {
  ...
  if (!db_dir.isExistsByName(prefixName, "system")) {
    ...
    // Adding icons
    db_icon.addIcon("", "winecfg.exe", "winecfg",
                    "Configure the general settings for Wine", prefixName,
                    "system", "winecfg");
    db_icon.addIcon("--backend=user cmd", "wineconsole", "wineconsole",
                    "Wineconsole is similar to wine command wcmd", prefixName,
                    "system", "wineconsole");
    db_icon.addIcon("", "uninstaller.exe", "uninstaller",
                    "Uninstall Windows programs under Wine properly",
                    prefixName, "system", "uninstaller");
    db_icon.addIcon("", "regedit.exe", "regedit", "Wine registry editor",
                    prefixName, "system", "regedit");
    db_icon.addIcon("", "explorer.exe", "explorer",
                    "Browse the files in the virtual Wine Drive", prefixName,
                    "system", "explorer");
    db_icon.addIcon("", "eject.exe", "eject", "Wine CD eject tool", prefixName,
                    "system", "eject");
    db_icon.addIcon("", "wordpad.exe", "wordpad", "Wine wordpad text editor",
                    prefixName, "system", "wordpad");
    db_icon.addIcon("", "taskmgr.exe", "taskmgr", "Wine task manager",
                    prefixName, "system", "taskmgr");
    db_icon.addIcon("", "winemine.exe", "winemine", "Wine saper game",
                    prefixName, "system", "winemine");
    db_icon.addIcon("", "oleview.exe", "wordpad", "Wine OLE/COM object viewer",
                    prefixName, "system", "oleview");
    db_icon.addIcon("", "notepad.exe", "notepad", "Wine notepad text editor",
                    prefixName, "system", "notepad");
    db_icon.addIcon("", "iexplore.exe", "iexplore", "Wine internet expolrer",
                    prefixName, "system", "iexplore");
    db_icon.addIcon("", "control.exe", "control", "Wine control panel",
                    prefixName, "system", "control");
  }
...
QString corelib::decodeRegString(const QString &string) {
  auto toUtf8 = QStringDecoder(QStringDecoder::Utf16BE);
  QString ret;
...
bool corelib::exportPrefixesConfiguration(void) {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (!file.exists(path)) {
      if (!file.link(prefix_path, path)) return false;
...
QStringList corelib::importPrefixesConfiguration(void) {
  ...
  for (int i = 0; i < p_list.size(); ++i) {
    ...
    if (!db_prefix.isExistsByName(prefix_name)) {
      ...
      if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return prefixes;
      ...
      if (wine_serv.isEmpty()) wine_serv = wine_bin;

      if (wine_loader.isEmpty()) wine_loader = wine_bin;

      if (!db_prefix.addPrefix(prefix_name, prefix_path, wine_bin, wine_serv,
                               wine_loader, wine_lib)) {
        qDebug() << "[EE] Cannot add prefix:" << prefix_name
                 << " into database..";
      } else {
...
#endif
      if (not QFile(list.at(i).absoluteFilePath()).remove()) return false;
    }
  }
  if (not dir.rmdir(dirPath)) return false;
  return true;
}
