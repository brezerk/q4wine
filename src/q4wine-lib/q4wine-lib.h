 ...
 * \brief libq4wine-core package provides general functions for q4wine-gui and
 * q4wine-cli packages.
 */
...
class corelib {
 public:
  ...
  /*! \brief This function tries to get wine process list running in the current
   * system.
   *
   * It gets process values: pid, name, nice priority and WINEPREFIX environment
   * variable
   * \note On Linux it uses /proc file system, and on FreeBSD/MacOS it uses
   * /dev/kmem and kvm.h functions.
   ...
   * \param  checkExist  If true - then we need to check exists file or dir by
   * value or not.
   * \return Return a key value.
   */
  QVariant getSetting(const QString &group, const QString &key,
   ...
   */
  QChar getCdromWineDrive(QString prefix_path, const QString &mount_point);
   ...
   */
  bool runIcon(const QString &prefix_name, const QString &dir_name,
   ...
   * \return Return true on success.
   */
  bool runWineBinary(const ExecObject &execObj, const QString &prefix_name,
                     const bool detach = true);
  // bool runWineBinary(const QString winebinary, const QString cmdargs, const
  // QString prefix_name, const QString wineAppendBin="", const bool releaseProc
  // = true);
  bool checkFileExists(const QString &path);

  QString createDesktopFile(const QString &prefix_name, const QString &dir_name,
                            const QString &icon_name,
                            const bool is_menu = false) const;
  bool deleteDesktopFile(const QString &prefix_name, const QString &dir_name,
   ...
   */
  bool runProcess(const QString &exec, const QStringList &args,
                  QString dir = "", const bool showLog = true);
  bool runProcess(const QStringList &args, const QString &caption,
   ...
   *
   * This function will export Q4Wine prefixes information according with
   * http://wiki.winehq.org/BottleSpec
   *
   * \return true on success.
   */
   ...
   *
   * This function will import wine's prefixes information according with
   * http://wiki.winehq.org/BottleSpec
   *
   * \return true on success.
   */
 ...
 private:
   ...
   * \param  info	   Define display type. If false - user interactive
   * message.
   * \return When using an interactive display type, this functions returns a
   * user selected value.
   */
