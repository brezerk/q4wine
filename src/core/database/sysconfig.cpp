...
QStringList Sysconfig::getProviderSubtypes(int provider_id) const {
  ...
  QSqlQuery query;
  query.prepare(
      "SELECT type FROM sysconfig WHERE provider_id=:provider_id GROUP BY "
      "type");
...
QList<SysconfigItem> Sysconfig::getItems(QString provider, QString type,
                                         int sort_order, QString filter) const {
  ...
  if (type.isEmpty()) {
    if (filter.isEmpty()) {
      sql =
          "SELECT id, name, icon, desc, is_installed, provider_id FROM "
          "sysconfig WHERE provider_id=(SELECT id FROM providers WHERE "
          "name=:provider) AND type IS NULL";
    } else {
      sql = QString(
                "SELECT id, name, icon, desc, is_installed, provider_id FROM "
                "sysconfig WHERE provider_id=(SELECT id FROM providers WHERE "
                "name=:provider) AND type IS NULL AND name LIKE \"%%1%\"")
                .arg(filter);
    }
    sql.append(order);
    query.prepare(sql);
    ...
    if (filter.isEmpty()) {
      sql =
          "SELECT id, name, icon, desc, is_installed, provider_id FROM "
          "sysconfig WHERE provider_id=(SELECT id FROM providers WHERE "
          "name=:provider) AND type=:type";
    } else {
      sql = QString(
                "SELECT id, name, icon, desc, is_installed, provider_id FROM "
                "sysconfig WHERE provider_id=(SELECT id FROM providers WHERE "
                "name=:provider) AND type=:type AND name LIKE \"%%1%\"")
                .arg(filter);
    }
    sql.append(order);
    query.prepare(sql);
...
bool Sysconfig::add_item(QString name, QString icon, QString desc, QString type,
                         int provider_id, bool is_installed) {
  QSqlQuery query;
  if (type.isEmpty()) {
    query.prepare(
        "INSERT INTO sysconfig(id, name, icon, type, desc, provider_id, "
        "is_installed) VALUES(NULL, :name, :icon, NULL, :desc, :provider_id, "
        ":is_installed);");
  } else {
    query.prepare(
        "INSERT INTO sysconfig(id, name, icon, type, desc, provider_id, "
        "is_installed) VALUES(NULL, :name, :icon, :type, :desc, :provider_id, "
        ":is_installed);");
    query.bindValue(":type", type);
  }
...
bool Sysconfig::drop_items(int provider_id) {
  QSqlQuery query;
  query.prepare(
      "DELETE FROM sysconfig WHERE provider_id=:provider_id AND type IS NOT "
      "NULL");
  query.bindValue(":provider_id", provider_id);
