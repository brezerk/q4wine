...
QStringList Last_Run_Icon::getByExec(const QString &exec) const {
  ...
  //                    0       1         2          3           4        5 6 7
  query.prepare(
      "SELECT wrkdir, override, winedebug, useconsole, display, cmdargs, "
      "desktop, nice, lang FROM last_run_icon WHERE exec=:exec");
...
bool Last_Run_Icon::addIcon(const QString &cmdargs, const QString &exec,
                            ...
                            const int nice, const QString &lang) const {
  QSqlQuery query;
  ...
  query.prepare(
      "INSERT INTO last_run_icon(override, winedebug, useconsole, display, "
      "cmdargs, exec, wrkdir, desktop, nice, lang) VALUES(:override, "
      ":winedebug, :useconsole, :display, :cmdargs, :exec, :wrkdir, :desktop, "
      ":nice, :lang);");
  ...
  if (query.isValid()) {
    if (query.value(0).toInt() > 8) {
      query.prepare(
          "DELETE FROM last_run_icon WHERE id=(SELECT MIN(id) FROM "
          "last_run_icon)");
