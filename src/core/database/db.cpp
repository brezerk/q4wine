...
DataBase::DataBase(QObject* parent) : QObject(parent) {
  ...
  if (!QSqlDatabase::drivers().contains("QSQLITE")) {
    QErr << "[EE] " << "Critical error" << " : "
         << "Unable to load SQLite database driver. You need to compile qt-sql "
  ...
  QString db_path =
      corelib::getAppConfigLocation(QStringList() << "db" << "generic.dat");
#ifdef DEBUG
  ...
  if (!db.open()) {
    QErr << "[EE] " << "Critical error" << " : "
         << QString("Cannot open database file: %1 ; Error is: %2")
                .arg(db_path)
...
bool DataBase::checkDb() {
  ...
  tables << "prefix" << "dir" << "icon" << "images" << "last_run_icon"
         << "logging" << "providers" << "sysconfig" << "versions";
  ...
  for (int i = 0; i < tables.size(); ++i) {
    ...
    if (db.record(table).isEmpty()) {
      if (table == "prefix") {
        if (!query.exec(
                "CREATE TABLE prefix (wine_dllpath TEXT, wine_loader TEXT, "
                "wine_server TEXT, wine_exec TEXT, cdrom_mount TEXT, id "
                "INTEGER PRIMARY KEY, name TEXT, path TEXT, version TEXT);"))
          return false;
        ...
        if (!query.exec("CREATE TABLE dir (id INTEGER PRIMARY KEY, name TEXT, "
                        "prefix_id NUMERIC);"))
          return false;
      } else if (table == "icon") {
        if (!query.exec("CREATE TABLE icon (wrkdir TEXT, override TEXT, "
                        "winedebug TEXT, useconsole NUMERIC, display TEXT, "
                        "cmdargs TEXT, exec TEXT, icon_path TEXT, desc TEXT, "
                        "desktop TEXT, nice TEXT, dir_id NUMERIC, id INTEGER "
                        "PRIMARY KEY, name TEXT, prefix_id NUMERIC);"))
          return false;
      } else if (table == "images") {
        if (!query.exec("CREATE TABLE images (id INTEGER PRIMARY KEY, name "
                        "TEXT, path TEXT);"))
          return false;
      } else if (table == "last_run_icon") {
        if (!query.exec("CREATE TABLE last_run_icon (wrkdir TEXT, override "
                        "TEXT, winedebug TEXT, useconsole NUMERIC, display "
                        "TEXT, cmdargs TEXT, exec TEXT, desktop TEXT, nice "
                        "TEXT, id INTEGER PRIMARY KEY);"))
          return false;
      } else if (table == "logging") {
        if (!query.exec(
                "CREATE TABLE logging (id INTEGER PRIMARY KEY, name TEXT, exit "
                "NUMERIC, stdout TEXT, prefix_id NUMERIC, date NUMERIC);"))
          return false;
      } else if (table == "providers") {
        if (!query.exec("CREATE TABLE providers (id INTEGER PRIMARY KEY, name "
                        "TEXT, icon TEXT);"))
          return false;

        query.prepare(
            "INSERT INTO providers(id, name, icon) VALUES(NULL, :name, "
            ":icon);");
        ...
        if (!query.exec(
                "CREATE TABLE sysconfig (id INTEGER PRIMARY KEY, name TEXT, "
                "type TEXT, icon TEXT, desc TEXT, provider_id INTEGER);"))
          return false;

        // System items
        query.prepare(
            "INSERT INTO sysconfig(id, name, icon, type, desc, provider_id) "
            "VALUES(NULL, :name, :icon, NULL, :desc, 1);");
        ...
        query.prepare(
            "INSERT INTO sysconfig(id, name, icon, type, desc, provider_id) "
            "VALUES(NULL, :name, :icon, NULL, :desc, 2);");
        ...
        if (!query.exec(
                "CREATE TABLE versions (wine_dllpath32 TEXT, wine_dllpath64 "
                "TEXT, wine_loader TEXT, wine_server TEXT, wine_exec TEXT, id "
        ...
        if (settings.value("configure", "no") == "yes") {
          ...
          vers.wine_dllpath32_ =
              settings.value("WineLibs32", QString()).toString();
          vers.wine_dllpath64_ =
              settings.value("WineLibs64", QString()).toString();
          settings.endGroup();
        }
        if (!vers.save()) return false;
      }
    }
  }
...
bool DataBase::fixup() {
  ...
  if (query.next()) {
    if (!query.exec(
            "UPDATE providers SET icon='regedit' WHERE icon='regedit.png'")) {
