...
#ifdef DEBUG
      qDebug() << QString(
                      "[EE] export_version mismatch! Expected \"%1\", but got "
                      "\"%2\".")
...
short int XmlParser::getPageCount(void) { return page_count; }

short int XmlParser::getPageCurrent(void) { return page_current; }

QList<WineAppDBInfo> XmlParser::getAppSearchInfoList() {
  return _APPDB_SEARCH_INFO;
}

WineAppDBInfo XmlParser::getAppSearchInfo() { return appdb_appinfo; }
...
QList<WineAppDBCategory> XmlParser::getAppsList() { return appdb_apps; }
...
void XmlParser::parseAppVersion(const QDomElement &element,
                                WineAppDBInfo &appinfo) {
  QDomNode node = element.firstChild();
...
void XmlParser::parseTestResults(const QDomElement &element,
                                 WineAppDBInfo &appinfo) {
  QDomNode node = element.firstChild();
  while (!node.isNull()) {
...
QString XmlParser::getChildNodeData(const QDomNode &childNode) {
  ...
  if (childNode.nodeType() == QDomNode::TextNode) {
    ...
    data.replace(QRegularExpression("(<br />\\s*)+"), "<br /><br />");
    data.replace(QRegularExpression("&nbsp;"), "");
    data.replace(QRegularExpression("(<br />)+\\s*$"), "");
    data.replace(QRegularExpression("^\\s*(<br />)+"), "");
  }
  return data;
}
