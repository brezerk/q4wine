...
HttpCore::HttpCore() {
  ...
  connect(http.get(), SIGNAL(finished(QNetworkReply*)), this,
          SLOT(readResponseHeader(QNetworkReply*)));
  ...
  user_agent = QString("%1/%2 (X11; %3 %4) xmlparser/%5")
                   .arg(APP_SHORT_NAME)
                   .arg(APP_VERS)
                   .arg(APP_HOST)
                   .arg(APP_ARCH)
...
void HttpCore::getAppDBXMLPage(QString host, short int port, QString page) {
  ...
  if (!this->getCacheFile(page)) {
    QString url =
        QString("https://%1:%2/%3").arg(host, QString::number(port), page);
...
bool HttpCore::getCacheFile(QString page) {
  ...
  cache_file.append(QCryptographicHash::hash(page.toUtf8().constData(),
...
QString HttpCore::getXMLReply() {
  QString cache_file = corelib::getAppCacheLocation();
  cache_file.append(QCryptographicHash::hash(this->page.toUtf8().constData(),
                                             QCryptographicHash::Md4)
                        .toHex());
  ...
  if (reply->error() == QNetworkReply::NoError) {
    ...
    if (status_code == 301 || status_code == 302) {
      ...
      QNetworkRequest request =
          QNetworkRequest(QUrl(possibleRedirectUrl.toString()));
      request.setHeader(QNetworkRequest::ContentTypeHeader,
                        "application/x-www-form-urlencoded");
      request.setHeader(QNetworkRequest::UserAgentHeader, user_agent);
      http->get(request);
