...
system_menu::system_menu() {
  ...
  base_directory =
      corelib::getGenericDataLocation(QStringList() << "desktop-directories");
  base_icon = corelib::getGenericDataLocation(QStringList() << "applications");
  base_menu = corelib::getGenericConfigLocation(
      QStringList() << "menus" << "applications-merged"
                    << QString("%1.menu").arg(APP_SHORT_NAME));
}

bool system_menu::add_dom_icons(QDomDocument &menu_xml, QDomElement &root,
                                const QString &prefix_name,
                                const QString &dir_name,
                                const QStringList &iconsList) {
  if (iconsList.count() <= 0) return true;
  ...
  for (int m = 0; m < iconsList.size(); ++m) {
    ...
    QDomText name_text = menu_xml.createTextNode(
        QString("%1-%2").arg(APP_SHORT_NAME).arg(ientry));
...
QDomElement system_menu::create_dom_menu(QDomDocument &menu_xml,
                                         QDomElement &root,
                                         const QString &prefix_name,
                                         const QString &dir_name) {
  QString entry;
  ...
  nroot.appendChild(name_tag);
  QDomText name_text =
      menu_xml.createTextNode(QString("%1-%2").arg(APP_SHORT_NAME).arg(entry));
  name_tag.appendChild(name_text);
  QDomElement dir_tag = menu_xml.createElement("Directory");
  nroot.appendChild(dir_tag);
  QDomText dir_text = menu_xml.createTextNode(
      QString("%1-%2.directory").arg(APP_SHORT_NAME).arg(entry));
...
QString system_menu::get_dir_path(const QString &prefix_name,
                                  const QString &dir_name) {
  QStringList path;
  path << base_icon;
  path << APP_SHORT_NAME;
...
QString system_menu::get_dirfile_path(const QString &prefix_name,
                                      const QString &dir_name) {
  QStringList path;
  QStringList name;
...
bool system_menu::move_dir_info(const QString &prefix_name,
  ...
  if (!dir.rename(dir_path, dir_path_new)) return false;
...
bool system_menu::remove_dir_info(const QString &prefix_name,
                                  const QString &dir_name) {
  QString file_path;
  QString dir_path;
  ...
  if (dir_name.isEmpty()) {
    ...
    QRegularExpression rx(
        QString("%1-%2-.*\\.directory").arg(APP_SHORT_NAME).arg(prefix_name));
    QFileInfoList list = dir.entryInfoList();
    for (int i = 0; i < list.size(); ++i) {
      QRegularExpressionMatch match = rx.match(list.at(i).fileName());
      if (match.hasMatch()) {
#ifdef DEBUG
...
bool system_menu::create_dir_info(const QString &prefix_name,
                                  const QString &dir_name) {
  QString name;
  QString icon;
  ...
  QDir dir(dir_path);
  if (!dir.exists()) {
    if (!dir.mkdir(dir_path)) return false;
  }
...
bool system_menu::writeXMLSystemMenu() {
  bool export_sysdir =
      CoreLib->getSetting("Plugins", "ExportSystemFolder", false, false)
          .toBool();
  menu_xml = QDomDocument(
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    for (int j = 0; j < subresult.size(); ++j) {
      ...
      QDomElement root_l2 =
          this->create_dom_menu(menu_xml, root_l1, list.at(i), subresult.at(j));
      QStringList iconsList =
          db_icon.getIconsList(list.at(i), subresult.at(j), "");
      this->add_dom_icons(menu_xml, root_l2, list.at(i), subresult.at(j),
                          iconsList);
    }
  }
...
bool system_menu::generateSystemMenu(const QString &prefix_name,
                                     const QString &dir_name) {
...
#ifdef DEBUG
  qDebug() << "[DD] system_menu::generateSystemMenu: " << prefix_name << " "
           << dir_name;
#endif
...
#ifdef DEBUG
  qDebug() << "[DD] system_menu::generateSystemMenu: export_sysdir "
           << export_sysdir;
#endif
  ...
  for (int i = 0; i < prefixes.size(); ++i) {
...
#ifdef DEBUG
    qDebug() << "[DD] system_menu::generateSystemMenu: on prefix: "
             << prefixes.at(i);
#endif
    ...
    for (int m = 0; m < iconsList.size(); ++m) {
#ifdef DEBUG
      qDebug() << "[DD] system_menu::generateSystemMenu: createDesktopFile "
               << prefixes.at(i) << " " << iconsList.at(m);
#endif
...
#ifdef DEBUG
      qDebug() << "[DD] system_menu::generateSystemMenu: on subdir: "
               << subdirs.at(j);
#endif
      this->create_dir_info(prefixes.at(i), subdirs.at(j));
      QStringList iconsList =
          db_icon.getIconsList(prefixes.at(i), subdirs.at(j), "");
      for (int z = 0; z < iconsList.size(); ++z) {
#ifdef DEBUG
        qDebug() << "[DD] system_menu::generateSystemMenu: createDesktopFile "
                 << prefixes.at(i) << " " << subdirs.at(j) << " "
                 << iconsList.at(z);
#endif
        CoreLib->createDesktopFile(prefixes.at(i), subdirs.at(j),
                                   iconsList.at(z), true);
      }
    }
  }
...
bool system_menu::wipeSystemMenu() {
  ...
  QDir dir(base_directory);
  QRegularExpression rx(QString("%1-.*\\.directory").arg(APP_SHORT_NAME));
  QFileInfoList list = dir.entryInfoList();
  for (int i = 0; i < list.size(); ++i) {
    QRegularExpressionMatch match = rx.match(list.at(i).fileName());
    if (match.hasMatch()) {
