...
FakeDriveSettings::FakeDriveSettings(QString prefixName, QWidget *parent,
                                     Qt::WindowFlags f)
    : QDialog(parent, f) {
  ...
  connect(optionsTree, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
          SLOT(optionsTree_itemClicked(QTreeWidgetItem *, int)));
  ...
  QList<QTreeWidgetItem *> items =
      optionsTree->findItems(tr("General"), Qt::MatchExactly);
  if (items.count() > 0) {
...
void FakeDriveSettings::cmdOk_Click() {
  ...
  QList<QListWidgetItem *> listItems = listJoystickAxesMappings->findItems(
  ...
  for (int i = 0; i < listItems.count(); i++) {
    QRegularExpressionMatch match = rx.match(listItems.at(i)->text());
    if (match.hasMatch()) {
      QMessageBox::warning(
          this, tr("Error"),
          tr("Error in string:\n\n%1\n\nJoystick axes mappings might be "
             "defined as:\n\"Joystick name\"=\"axes mapping\"\n\nFor "
             "example:\n\"Logitech Logitech Dual "
             "Action\"=\"X,Y,Rz,Slider1,POV1\"\n\nSee help for details.")
              .arg(listItems.at(i)->text()));
      return;
    }
  }
  ...
  if (comboFakeVersion->currentText() == "Windows XP") {
    version = "winxp";
  } else if (comboFakeVersion->currentText() == "Windows 11") {
    version = "win11";
  } else if (comboFakeVersion->currentText() == "Windows 10") {
    version = "win10";
    ...
    version = "win7";
  } else if (comboFakeVersion->currentText() == "Windows 2008r2") {
      version = "win2008r2";
  ...
  QStringList list;
  list << "\"Desktop\"" << "\"My Music\"" << "\"My Pictures\""
       << "\"My Videos\"" << "\"Personal\"";
  list = reg.readKeys(
      "user",
  ...
  if (list.count() == 5) {
    desktopFolder = QString("%1/%2").arg(userDir).arg(
        CoreLib->decodeRegString(list.at(0).split("\\\\").last()));
    desktopMusic = QString("%1/%2").arg(userDir).arg(
        CoreLib->decodeRegString(list.at(1).split("\\\\").last()));
    desktopPictures = QString("%1/%2").arg(userDir).arg(
        CoreLib->decodeRegString(list.at(2).split("\\\\").last()));
    desktopDocuments = QString("%1/%2").arg(userDir).arg(
        CoreLib->decodeRegString(list.at(4).split("\\\\").last()));
    desktopVideos = QString("%1/%2").arg(userDir).arg(
        CoreLib->decodeRegString(list.at(3).split("\\\\").last()));
  } else {
...
#ifdef DEBUG
  qDebug() << "[ii] Localized folders path: " << desktopFolder
           << desktopDocuments << desktopMusic << desktopPictures
           << desktopVideos;
#endif
    ...
    for (int i = 0; i < drivelist.size(); ++i) {
      ...
      if (fileInfo.isSymLink()) {
        if (!file.remove(fileInfo.filePath())) {
          QMessageBox::warning(
              this, tr("Error"),
              tr("Cannot remove symlink: %1").arg(fileInfo.filePath()));
  ...
  if (listWineDrives->count() > 0) {
    for (int i = 0; i < listWineDrives->count(); i++) {
      std::unique_ptr<DriveListWidgetItem> item(
          dynamic_cast<DriveListWidgetItem *>(listWineDrives->item(i)));

      if (item.get()) {
        QString driveLink =
            QString("%1/%2").arg(prefixPath).arg(item->getLetter().toLower());
        if (!file.link(item->getPath(), driveLink)) {
          QMessageBox::warning(this, tr("Error"),
                               tr("Cannot symlink \"%1\" to \"%2\"")
                                   .arg(item->getPath())
                                   .arg(driveLink));
          QApplication::restoreOverrideCursor();
          reject();
  ...
  if (!dir.exists(prefixPath)) {
    if (!dir.mkdir(prefixPath)) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Cannot create dir: %1").arg(prefixPath));
      QApplication::restoreOverrideCursor();
      reject();
      return;
    }
  ...
  if (fileInfo.isSymLink() || fileInfo.exists()) {
    if (!dir.remove(desktopFolder)) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Cannot remove dir: %1").arg(desktopFolder));
      QApplication::restoreOverrideCursor();
      reject();
      return;
    }
...
\"%1\"").arg(desktopFolder)); QApplication::restoreOverrideCursor(); reject();
        return;
    }
} else {*/
  ...
  if (!txtWineDesktopDoc->text().isEmpty()) {
    ...
    if (fileInfo.isSymLink() || fileInfo.exists()) {
      if (!dir.remove(desktopDocuments)) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Cannot remove dir: %1").arg(desktopDocuments));
        QApplication::restoreOverrideCursor();
        reject();
        return;
      }
    ...
    if (!file.link(txtWineDesktopDoc->text(), desktopDocuments)) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Cannot symlink \"%1\" to \"%2\"")
                               .arg(txtWineDesktopDoc->text())
                               .arg(desktopDocuments));
      QApplication::restoreOverrideCursor();
      reject();
      return;
    }
  ...
  if (!txtWineDesktopMus->text().isEmpty()) {
    ...
    if (fileInfo.isSymLink() || fileInfo.exists()) {
      if (!dir.remove(desktopMusic)) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Cannot remove dir: %1").arg(desktopMusic));
        QApplication::restoreOverrideCursor();
        reject();
        return;
      }
  ...
  if (!txtWineDesktopPic->text().isEmpty()) {
    ...
    if (fileInfo.isSymLink() || fileInfo.exists()) {
      if (!dir.remove(desktopPictures)) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Cannot remove dir: %1").arg(desktopPictures));
        QApplication::restoreOverrideCursor();
        reject();
        return;
      }
  ...
  if (!txtWineDesktopVid->text().isEmpty()) {
    ...
    if (fileInfo.isSymLink() || fileInfo.exists()) {
      if (!dir.remove(desktopVideos)) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Cannot remove dir: %1").arg(desktopVideos));
        QApplication::restoreOverrideCursor();
        reject();
        return;
      }
...
#endif
    registry.set("Software\\Wow6432Node\\Microsoft\\Windows NT\\CurrentVersion",
                 "RegisteredOrganization", txtOrganization->text(),
                 "HKEY_LOCAL_MACHINE");
    registry.set("Software\\Wow6432Node\\Microsoft\\Windows NT\\CurrentVersion",
                 "RegisteredOwner", txtOwner->text(), "HKEY_LOCAL_MACHINE");
  } else {
#ifdef DEBUG
    qDebug() << "[ii] Writedown wine32 settings.";
#endif
    registry.set("Software\\Microsoft\\Windows NT\\CurrentVersion",
                 "RegisteredOrganization", txtOrganization->text(),
                 "HKEY_LOCAL_MACHINE");
    registry.set("Software\\Microsoft\\Windows NT\\CurrentVersion",
                 "RegisteredOwner", txtOwner->text(), "HKEY_LOCAL_MACHINE");
  ...
  if (cbCrashDialog->isChecked()) {
    registry.set("Software\\Wine\\WineDbg", "ShowCrashDialog",
                 "dword:00000000");
  } else {
  ...
  if (listWineDrives->count() > 0) {
    ...
    for (int i = 0; i < listWineDrives->count(); i++) {
      std::unique_ptr<DriveListWidgetItem> item(
          dynamic_cast<DriveListWidgetItem *>(listWineDrives->item(i)));
#ifdef DEBUG
      qDebug() << "[ii] Wizard::get DriveListWidgetItem for "
               << listWineDrives->item(i)->text();
#endif
      if (item.get()) {
        if (item->getType() == "auto") {
          registry.unset("Software\\Wine\\Drives", item->getLetter().toLower(),
                         "HKEY_LOCAL_MACHINE");
        } else {
          registry.set("Software\\Wine\\Drives", "",
                       QString("\"%1\"=\"%2\"")
                           .arg(item->getLetter().toLower())
                           .arg(item->getType()),
                       "HKEY_LOCAL_MACHINE");
        }
      }
  ...
  if (!txtFakeBrowsers->text().isEmpty()) {
    registry.set("Software\\Wine\\WineBrowser", "Browsers",
                 txtFakeBrowsers->text());
  } else {
    registry.unset("Software\\Wine\\WineBrowser", "Browsers");
  }

  if (!txtFakeMailers->text().isEmpty()) {
    registry.set("Software\\Wine\\WineBrowser", "Mailers",
                 txtFakeMailers->text());
  } else {
  ...
  if (comboFakeD3D_Multi->currentText() != "default") {
    registry.set("Software\\Wine\\Direct3D", "Multisampling",
                 comboFakeD3D_Multi->currentText());
  } else {
    registry.unset("Software\\Wine\\Direct3D", "Multisampling");
  }

  if (comboFakeD3D_Render->currentText() != "default") {
    registry.set("Software\\Wine\\Direct3D", "DirectDrawRenderer",
                 comboFakeD3D_Render->currentText());
  } else {
    registry.unset("Software\\Wine\\Direct3D", "DirectDrawRenderer");
  }

  if (comboFakeD3D_LMode->currentText() != "default") {
    registry.set("Software\\Wine\\Direct3D", "RenderTargetLockMode",
                 comboFakeD3D_LMode->currentText());
  } else {
    registry.unset("Software\\Wine\\Direct3D", "RenderTargetLockMode");
  }

  if (comboFakeD3D_SDOrder->currentText() != "default") {
    registry.set("Software\\Wine\\Direct3D", "StrictDrawOrdering",
                 comboFakeD3D_SDOrder->currentText());
  } else {
    registry.unset("Software\\Wine\\Direct3D", "StrictDrawOrdering");
  }

  if (comboFakeD3D_Offscreen->currentText() != "default") {
    registry.set("Software\\Wine\\Direct3D", "OffscreenRenderingMode",
                 comboFakeD3D_Offscreen->currentText());
  } else {
  ...
  if (listJoystickAxesMappings->count() > 0) {
    ...
    for (int i = 0; i < listJoystickAxesMappings->count(); i++) {
      registry.set("Software\\Wine\\DirectInput", "",
                   listJoystickAxesMappings->item(i)->text());
    }
  }
  ...
  if (comboFakeMouseWarp->currentText() != "default") {
    registry.set("Software\\Wine\\DirectInput", "MouseWarpOverride",
                 comboFakeMouseWarp->currentText());
  } else {
    registry.unset("Software\\Wine\\DirectInput", "MouseWarpOverride");
  }

  if (comboFakeX11_WR->currentText() != "default") {
    registry.set("Software\\Wine\\X11 Driver", "ClientSideWithRender",
                 comboFakeX11_WR->currentText());
  } else {
    registry.unset("Software\\Wine\\X11 Driver", "ClientSideWithRender");
  }

  if (comboFakeX11_AAR->currentText() != "default") {
    registry.set("Software\\Wine\\X11 Driver", "ClientSideAntiAliasWithRender",
                 comboFakeX11_AAR->currentText());
  } else {
    registry.unset("Software\\Wine\\X11 Driver",
                   "ClientSideAntiAliasWithRender");
  }

  if (comboFakeX11_AAC->currentText() != "default") {
    registry.set("Software\\Wine\\X11 Driver", "ClientSideAntiAliasWithCore",
                 comboFakeX11_AAC->currentText());
  } else {
    registry.unset("Software\\Wine\\X11 Driver", "ClientSideAntiAliasWithCore");
  }

  if (comboFakeX11_XRandr->currentText() != "default") {
    registry.set("Software\\Wine\\X11 Driver", "UseXRandR",
                 comboFakeX11_XRandr->currentText());
  } else {
    registry.unset("Software\\Wine\\X11 Driver", "UseXRandR");
  }

  if (comboFakeX11_XVid->currentText() != "default") {
    registry.set("Software\\Wine\\X11 Driver", "UseXVidMode",
                 comboFakeX11_XVid->currentText());
  } else {
  ...
  if (comboFakeSound_Driver->currentText() != "default") {
    if (comboFakeSound_Driver->currentText() == "disabled") {
      ...
      registry.set("Software\\Wine\\Drivers", "Audio",
                   comboFakeSound_Driver->currentText());
    }
  } else {
  ...
  if (comboFakeAlsa_asCards->currentText() != "default") {
    registry.set("Software\\Wine\\Alsa Driver", "AutoScanCards",
                 comboFakeAlsa_asCards->currentText());
  } else {
    registry.unset("Software\\Wine\\Alsa Driver", "AutoScanCards");
  }

  if (comboFakeAlsa_asDevices->currentText() != "default") {
    registry.set("Software\\Wine\\Alsa Driver", "AutoScanDevices",
                 comboFakeAlsa_asDevices->currentText());
  } else {
    registry.unset("Software\\Wine\\Alsa Driver", "AutoScanDevices");
  }

  if (sboxFakeAlsa_devCount->value() > 0) {
    registry.set("Software\\Wine\\Alsa Driver", "DeviceCount",
                 QString("%1").arg(sboxFakeAlsa_devCount->value()));
  } else {
    registry.unset("Software\\Wine\\Alsa Driver", "DeviceCount");
  }

  if (!txtFakeAlsa_CTLn->text().isEmpty()) {
    registry.set("Software\\Wine\\Alsa Driver", "DeviceCTLn",
                 txtFakeAlsa_CTLn->text());
  } else {
    registry.unset("Software\\Wine\\Alsa Driver", "DeviceCTLn");
  }

  if (!txtFakeAlsa_PCMn->text().isEmpty()) {
    registry.set("Software\\Wine\\Alsa Driver", "DevicePCMn",
                 txtFakeAlsa_PCMn->text());
  } else {
    registry.unset("Software\\Wine\\Alsa Driver", "DevicePCMn");
  }

  if (comboFakeAlsa_DirectHW->currentText() != "default") {
    registry.set("Software\\Wine\\Alsa Driver", "UseDirectHW",
                 comboFakeAlsa_DirectHW->currentText());
  } else {
    registry.unset("Software\\Wine\\Alsa Driver", "UseDirectHW");
  }

  if (sboxFakeSound_shadow->value() > -1) {
    registry.set("Software\\Wine\\DirectSound", "MaxShadowSize",
                 QString("%1").arg(sboxFakeSound_shadow->value()));
  } else {
  ...
  if (rbColorsQt4->isChecked()) {
    ...
    registry.set("Control Panel\\Colors", "Window",
                 QString("%1 %2 %3")
    ...
    registry.set("Control Panel\\Colors", "ActiveBorder",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "InactiveBorder",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "AppWorkSpace",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "Menu",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "MenuBar",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "Scrollbar",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "MenuHilight",
                 QString("%1 %2 %3")
                     ...
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "ButtonFace",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "ButtonAlternateFace",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "ButtonDkShadow",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "ButtonShadow",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "ButtonHilight",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "ButtonText",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "MenuText",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "WindowFrame",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    registry.set("Control Panel\\Colors", "WindowText",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "Hilight",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "HilightText",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "InfoWindow",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
    ...
    registry.set("Control Panel\\Colors", "InfoText",
                 QString("%1 %2 %3")
                     .arg(QString::number(color.red()))
                     .arg(QString::number(color.green()))
                     .arg(QString::number(color.blue())));
  ...
  if ((!curr_text.isEmpty()) and (!curr_text.isNull()) and
      (!last_items.contains(curr_text)))
...
void FakeDriveSettings::loadThemeIcons() {
  connect(cmdJoystickEdit, SIGNAL(clicked()), this,
          SLOT(cmdJoystickEdit_Click()));
  connect(cmdJoystickAdd, SIGNAL(clicked()), this,
          SLOT(cmdJoystickAdd_Click()));
  connect(cmdJoystickDel, SIGNAL(clicked()), this,
          SLOT(cmdJoystickDel_Click()));
  connect(cmdWineDriveEdit, SIGNAL(clicked()), this,
          SLOT(cmdWineDriveEdit_Click()));
  connect(cmdWineDriveAdd, SIGNAL(clicked()), this,
          SLOT(cmdWineDriveAdd_Click()));
  connect(cmdWineDriveDel, SIGNAL(clicked()), this,
          SLOT(cmdWineDriveDel_Click()));
...
void FakeDriveSettings::cmdJoystickEdit_Click() {
  ...
  QString text = QInputDialog::getText(
      this, tr("Joystick Axes Mappings"),
      tr("Joystick axes mappings might be defined as:\n\"Joystick "
         "name\"=\"axes mapping\"\n\nFor example:\n\"Logitech Logitech Dual "
         "Action\"=\"X,Y,Rz,Slider1,POV1\"\n\nSee help for details."),
...
void FakeDriveSettings::cmdJoystickDel_Click() {
  std::unique_ptr<QListWidgetItem> item(
      listJoystickAxesMappings->currentItem());
  if (!item.get()) return;
...
void FakeDriveSettings::cmdWineDriveEdit_Click() {
  std::unique_ptr<DriveListWidgetItem> item(
      dynamic_cast<DriveListWidgetItem *>(listWineDrives->currentItem()));
  ...
  WineDriveDialog drevedialog(drives, item->getLetter(), item->getPath(),
                              item->getType());
  if (drevedialog.exec() == QDialog::Accepted) {
    item->setDrive(drevedialog.getLetter(), drevedialog.getPath(),
                   drevedialog.getType());
  }
  item.release();
  return;
...
void FakeDriveSettings::cmdWineDriveAdd_Click() {
  ...
  if (drevedialog.exec() == QDialog::Accepted) {
    std::unique_ptr<DriveListWidgetItem> item(
        new DriveListWidgetItem(listWineDrives));
    item->setDrive(drevedialog.getLetter(), drevedialog.getPath(),
                   drevedialog.getType());
...
void FakeDriveSettings::cmdWineDriveDel_Click() {
  ...
  if (item->text().left(2) == "C:") {
    QMessageBox::warning(
        this, tr("Error"),
        tr("Sorry, You cannot delete or modify wine C: drive.<br>But You can "
           "change it in prefix settings dialog."));
    item.release();
    return;
  }
...
void FakeDriveSettings::loadSettings() {
  ...
  list = reg.readKeys(
      "system", "Software\\Wow6432Node\\Microsoft\\Windows NT\\CurrentVersion",
      list);
  ...
  if (txtOrganization->text().isEmpty() && txtOwner->text().isEmpty()) {
    ...
    list << "\"RegisteredOrganization\"" << "\"RegisteredOwner\"";
    list = reg.readKeys(
        "system", "Software\\Microsoft\\Windows NT\\CurrentVersion", list);
  ...
  if (list.count() > 0) {
    ...
    if (version == "winxp") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows XP"));
    } else if (version == "win11") {
        comboFakeVersion->setCurrentIndex(
            comboFakeVersion->findText("Windows 11"));
    } else if (version == "win10") {
        comboFakeVersion->setCurrentIndex(
            comboFakeVersion->findText("Windows 10"));
    } else if (version == "win81") {
        comboFakeVersion->setCurrentIndex(
            comboFakeVersion->findText("Windows 8.1"));
    } else if (version == "win8") {
        comboFakeVersion->setCurrentIndex(
            comboFakeVersion->findText("Windows 8"));
    } else if (version == "win7") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 7"));
    } else if (version == "win2008r2") {
        comboFakeVersion->setCurrentIndex(
            comboFakeVersion->findText("Windows 2008r2"));
    } else if (version == "win2008") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 2008"));
    } else if (version == "vista") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows Vista"));
    } else if (version == "win2003") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 2003"));
    } else if (version == "win2k") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 2000"));
    } else if (version == "winme") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows ME"));
    } else if (version == "win98") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 98"));
    } else if (version == "win95") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 95"));
    } else if (version == "nt40") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows NT 4.0"));
    } else if (version == "nt351") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows NT 3.0"));
    } else if (version == "win31") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 3.1"));
    } else if (version == "win30") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 3.0"));
    } else if (version == "win20") {
      comboFakeVersion->setCurrentIndex(
          comboFakeVersion->findText("Windows 2.0"));
    } else {
        qDebug() << "[ii] Wizard::unknownVersion " << version;
    }
  }
  ...
  if (list.count() > 0) {
    if (!list.at(0).isEmpty()) cbCrashDialog->setChecked(true);
  }
  ...
  list.clear();
  list << "\"Multisampling\"" << "\"DirectDrawRenderer\""
       << "\"RenderTargetLockMode\"" << "\"OffscreenRenderingMode\""
       << "\"UseGLSL\"" << "\"VideoMemorySize\"" << "\"VideoDescription\""
       << "\"VideoDriver\"" << "\"SoftwareEmulation\"" << "\"PixelShaderMode\""
       << "\"VertexShaderMode\"" << "\"StrictDrawOrdering\""
       << "\"Nonpower2Mode\"";
  list = reg.readKeys("user", "Software\\Wine\\Direct3D", list);

  if (list.count() > 0) {
    if (!list.at(0).isEmpty())
      comboFakeD3D_Multi->setCurrentIndex(
          comboFakeD3D_Multi->findText(list.at(0)));

    if (!list.at(1).isEmpty())
      comboFakeD3D_Render->setCurrentIndex(
          comboFakeD3D_Render->findText(list.at(1)));

    if (!list.at(2).isEmpty())
      comboFakeD3D_LMode->setCurrentIndex(
          comboFakeD3D_LMode->findText(list.at(2)));

    if (!list.at(3).isEmpty())
      comboFakeD3D_Offscreen->setCurrentIndex(
          comboFakeD3D_Offscreen->findText(list.at(3)));

    if (!list.at(4).isEmpty())
      comboFakeD3D_GLSL->setCurrentIndex(
          comboFakeD3D_GLSL->findText(list.at(4)));
    ...
    if (!list.at(8).isEmpty())
      comboFakeSoftwareEmulation->setCurrentIndex(
          comboFakeSoftwareEmulation->findText(list.at(8)));

    if (!list.at(9).isEmpty())
      comboFakePixelShaderMode->setCurrentIndex(
          comboFakePixelShaderMode->findText(list.at(9)));

    if (!list.at(10).isEmpty())
      comboFakeVertexShaderMode->setCurrentIndex(
          comboFakeVertexShaderMode->findText(list.at(10)));

    if (!list.at(11).isEmpty())
      comboFakeD3D_SDOrder->setCurrentIndex(
          comboFakeD3D_SDOrder->findText(list.at(11)));

    if (!list.at(12).isEmpty())
      comboFakeD3D_N2M->setCurrentIndex(
          comboFakeD3D_N2M->findText(list.at(12)));
  }
  ...
  if (list.count() > 0) {
    if (!list.at(0).isEmpty())
      comboFakeMouseWarp->setCurrentIndex(
          comboFakeMouseWarp->findText(list.at(0)));
  }
  ...
  if (list.count() > 0) {
    if (!list.at(0).isEmpty())
      comboFakeInput_selection->setCurrentIndex(
          comboFakeInput_selection->findText(list.at(0)));
  }

  list.clear();
  list << "\"ClientSideWithRender\"" << "\"ClientSideAntiAliasWithRender\""
       << "\"ClientSideAntiAliasWithCore\"" << "\"UseXRandR\""
       << "\"UseXVidMode\"";
  list = reg.readKeys("user", "Software\\Wine\\X11 Driver", list);
  ...
  if (list.count() > 0) {
    ...
    if (!list.at(3).isEmpty())
      comboFakeX11_XRandr->setCurrentIndex(
          comboFakeX11_XRandr->findText(list.at(3)));

    if (!list.at(4).isEmpty())
      comboFakeX11_XVid->setCurrentIndex(
          comboFakeX11_XVid->findText(list.at(4)));
  }
  ...
  if (list.count() > 0) {
    if (list.at(0) == QString()) {
      comboFakeSound_Driver->setCurrentIndex(
          comboFakeSound_Driver->findText("default"));
    } else if (!list.at(0).isEmpty()) {
      comboFakeSound_Driver->setCurrentIndex(
          comboFakeSound_Driver->findText(list.at(0)));
    } else {
      comboFakeSound_Driver->setCurrentIndex(
          comboFakeSound_Driver->findText("disabled"));
    }
  }

  list.clear();
  list << "\"AutoScanCards\"" << "\"AutoScanDevices\"" << "\"DeviceCount\""
       << "\"DeviceCTLn\"" << "\"DevicePCMn\"" << "\"UseDirectHW\"";
  list = reg.readKeys("user", "Software\\Wine\\Alsa Driver", list);

  if (list.count() > 0) {
    if (!list.at(0).isEmpty())
      comboFakeAlsa_asCards->setCurrentIndex(
          comboFakeAlsa_asCards->findText(list.at(0)));

    if (!list.at(1).isEmpty())
      comboFakeAlsa_asDevices->setCurrentIndex(
          comboFakeAlsa_asDevices->findText(list.at(1)));
    ...
    if (!list.at(5).isEmpty())
      comboFakeAlsa_DirectHW->setCurrentIndex(
          comboFakeAlsa_DirectHW->findText(list.at(5)));
  }
  ...
  if (!wineDriveDir.cd(prefixPath)) {
    ...
    fileinfo.setFile(
        QString("%1/%2").arg(prefixPath).arg(this->desktopDocuments));
    if (fileinfo.isSymLink()) {
    ...
    fileinfo.setFile(
        QString("%1/%2").arg(prefixPath).arg(this->desktopPictures));
    if (fileinfo.isSymLink()) {
...
void FakeDriveSettings::loadDefaultSettings() {
  ...
  if ((!mount_point.isEmpty()) and (!mount_point_windrive.isNull())) {
    item.reset(new DriveListWidgetItem(listWineDrives));
    item->setDrive(QString("%1:").arg(mount_point_windrive), mount_point,
                   "cdrom");
    listWineDrives->addItem(item.release());
  }
  ...
  QString wineDesktopPath =
      CoreLib
          ->getSetting(
              "DesktopIntegration", "WineDesktop", false,
              QString("%1/desktop-integration/Desktop").arg(prefixPath))
          .toString();
  ...
  wineDesktopPath =
      CoreLib
          ->getSetting("DesktopIntegration", "WineDesktopDoc", false,
                       QString("%1/desktop-integration/").arg(prefixPath))
          .toString();
  if (wineDesktopPath.isEmpty()) {
    wineDesktopPath = QString("%1/desktop-integration/Desktop").arg(prefixPath);
  }
  ...
  wineDesktopPath =
      CoreLib
          ->getSetting("DesktopIntegration", "WineDesktopMus", false,
                       QString("%1/desktop-integration/").arg(prefixPath))
          .toString();
  if (wineDesktopPath.isEmpty()) {
  ...
  wineDesktopPath =
      CoreLib
          ->getSetting("DesktopIntegration", "WineDesktopPic", false,
                       QString("%1/desktop-integration/").arg(prefixPath))
          .toString();
  if (wineDesktopPath.isEmpty()) {
    wineDesktopPath = QString("%1/desktop-integration/").arg(prefixPath);
  ...
  wineDesktopPath =
      CoreLib
          ->getSetting("DesktopIntegration", "WineDesktopVid", false,
                       QString("%1/desktop-integration/").arg(prefixPath))
          .toString();
  if (wineDesktopPath.isEmpty()) {
    wineDesktopPath = QString("%1/desktop-integration/").arg(prefixPath);
...
bool FakeDriveSettings::eventFilter(QObject *obj, QEvent *event) {
  ...
  */
  ...
  if (obj->objectName() == "FakeDriveSettings") return false;
  ...
  if (event->type() == QEvent::MouseButtonRelease) {
    ...
    if (QString("%1").arg(
            findChild<QWidget *>(obj_name)->metaObject()->className()) ==
        "QComboBox") {
