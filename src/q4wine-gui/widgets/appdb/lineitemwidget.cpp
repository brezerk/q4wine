...
LineItemWidget::LineItemWidget(const short int action, QWidget *parent)
    : QWidget(parent) {
...
void LineItemWidget::addLabel(const QString text, const short int width,
                              const short int alignment, const bool worldwarp) {
...
void LineItemWidget::insertStretch(void) { contentLayout->insertStretch(-1); }
...
bool LineItemWidget::eventFilter(QObject *obj, QEvent *event) {
  ...
  if (event->type() == QEvent::Enter) {
    QPalette p(palette());
    // Set colour
    p.setColor(QPalette::Base, QPalette().color(QPalette::Highlight));
    p.setColor(QPalette::WindowText,
               QPalette().color(QPalette::HighlightedText));
    this->setPalette(p);
...
void LineItemWidget::setEnabled(bool enabled) {
  ...
  if (!enabled) {
    ...
    p.setColor(QPalette::Base, QPalette().color(QPalette::Base));
  } else {
    setCursor(Qt::PointingHandCursor);
    installEventFilter(this);
    p.setColor(QPalette::Base, QPalette().color(QPalette::Button));
  }
  this->setPalette(p);
