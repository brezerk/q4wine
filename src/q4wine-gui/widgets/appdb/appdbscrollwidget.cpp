...
void AppDBScrollWidget::addSearchWidget(const WineAppDBInfo appinfo) {
  if (contentLayout.get()) {
    std::unique_ptr<AppInfoWidget> AppDBWidget(new AppInfoWidget(
        appinfo.name, appinfo.desc, appinfo.id, appinfo.versions));
    connect(AppDBWidget.get(),
            SIGNAL(itemTrigged(short int, QString, int, int, int)), this,
            SIGNAL(itemTrigged(short int, QString, int, int, int)));
...
void AppDBScrollWidget::addTestWidget(const WineAppDBInfo appinfo) {
  if (contentLayout.get()) {
    ...
    connect(AppDBTestWidget.get(),
            SIGNAL(itemTrigged(short int, QString, int, int, int)), this,
            SIGNAL(itemTrigged(short int, QString, int, int, int)));
    connect(AppDBTestWidget.get(), SIGNAL(scrollToPos(int)), this,
            SLOT(scrollToPos(int)));
...
void AppDBScrollWidget::scrollToPos(int pos) {
  ...
  if (pos > 0) this->ensureVisible(0, pos, 0, this->height() / 3);
}
...
#ifdef DEBUG
      qDebug() << "[ii] Schedule QObject for deletetion. object type is:"
...
void AppDBScrollWidget::insertStretch(void) {
  if (contentLayout.get()) {
    ...
    visibleStrech->setSizePolicy(QSizePolicy::Preferred,
                                 QSizePolicy::Expanding);
    contentLayout->addWidget(visibleStrech.release());
  }
}

void AppDBScrollWidget::addVersionFrame(QList<WineAppDBCategory> list,
                                        QString frame_caption,
                                        short int action) {
  if (list.count() <= 0) return;

  std::unique_ptr<QGroupBox> frame(
      new QGroupBox(QString("%1:").arg(frame_caption)));
