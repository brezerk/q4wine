...
AppTestWidget::AppTestWidget(const WineAppDBInfo appinfo, QWidget *parent)
    : QWidget(parent) {
  setupUi(this);
  ...
  if (!appinfo.url.isEmpty()) {
    std::unique_ptr<LinkItemWidget> label(
        new LinkItemWidget("App web page", 6));
...
void AppTestWidget::requestParentComment(int id) {
  ...
  if (y_pos > 0) emit(scrollToPos(y_pos));
...
void AppTestWidget::addBugs(QList<WineAppDBBug> bugs) {
  if (bugs.count() == 0) grpKnownBugs->setVisible(false);
...
void AppTestWidget::addComments(QList<WineAppDBComment> comments) {
  for (int i = 0; i < comments.count(); i++) {
    ...
    connect(comment.get(), SIGNAL(requestParentComment(int)), this,
            SLOT(requestParentComment(int)));
...
void AppTestWidget::addTestResults(QList<WineAppDBTestResult> tests) {
  if (tests.count() == 0) grpTestResults->setVisible(false);
