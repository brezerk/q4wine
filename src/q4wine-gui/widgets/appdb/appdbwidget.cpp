...
AppDBWidget::AppDBWidget(QWidget *parent) : QWidget(parent) {
  ...
  connect(appdbHeader.get(),
          SIGNAL(itemTrigged(short int, QString, int, int, int)), this,
          SLOT(itemTrigged(short int, QString, int, int, int)));
  appdbScrollArea.reset(new AppDBScrollWidget());
  connect(appdbScrollArea.get(),
          SIGNAL(itemTrigged(short int, QString, int, int, int)), this,
          SLOT(itemTrigged(short int, QString, int, int, int)));
  ...
  connect(httpcore.get(), SIGNAL(pageReaded()), this,
          SLOT(httpcore_pageDownloaded()));
  connect(httpcore.get(), SIGNAL(requestError(QString)), this,
          SLOT(requestError(QString)));
  connect(httpcore.get(), SIGNAL(updateDataReadProgress(int, int)), this,
          SLOT(updateDataReadProgress(int, int)));
  connect(httpcore.get(), SIGNAL(stateChanged(int)), this,
...
void AppDBWidget::itemTrigged(short int action, QString search, int val1,
                              int val2, int val3) {
#ifdef DEBUG
  ...
  switch (action) {
    case 1:
      ...
      this->appdbHeader->addLabel(
          tr("Status: Connecting to %1").arg(APPDB_HOSTNAME));
      this->search = search;
      this->page = 0;
      this->appid = 0;
      this->verid = 0;
      ...
      if (searchField->text() != search) searchField->setText(search);
    ...
    case 4:
      if (CoreLib->getSetting("AppDB", "useSystemBrowser", false, 1) == 1) {
        QDesktopServices::openUrl(
            QUrl(QString("http://%1/objectManager.php?sClass=version&iId=%2")
                     .arg("appdb.winehq.org")
                     .arg(val2),
                 QUrl::TolerantMode));
      } else {
        this->setCursor(QCursor(Qt::WaitCursor));
    ...
    case 5:
      ...
      break;
    case 6:
      if (search.contains(QRegularExpression("^http"))) {
        QDesktopServices::openUrl(QUrl(search, QUrl::TolerantMode));
      } else {
        QDesktopServices::openUrl(
            QUrl(QString("http://%1").arg(search), QUrl::TolerantMode));
      }
      break;
    case 7:
      QDesktopServices::openUrl(QUrl(
          QString("http://%1/show_bug.cgi?id=%2").arg(BUGS_HOSTNAME).arg(val1),
          QUrl::TolerantMode));
      break;
  }
...
void AppDBWidget::httpcore_pageDownloaded(void) {
  ...
  switch (this->action) {
    case 1:
      appdbHeader->createPagesList(xmlparser->getPageCount(),
                                   xmlparser->getPageCurrent(), this->search);
    ...
    case 3:
      ...
      break;
    case 4:
      this->appdbHeader->createCategoryList(
          xmlparser->getAppSearchInfo().categorys);
      this->appdbHeader->addLabel(">");
      this->appdbHeader->addLink(xmlparser->getAppSearchInfo().name, true, 3,
                                 "", xmlparser->getAppSearchInfo().id);
      this->appdbHeader->addLabel(
          QString("- %1").arg(xmlparser->getAppSearchInfo().appver));
    ...
    case 5:
      ...
      this->appdbHeader->insertStretch();
      this->appdbScrollArea->addVersionFrame(xmlparser->getSubCategorysList(),
                                             tr("Subcategories"), 5);
      this->appdbScrollArea->addVersionFrame(xmlparser->getAppsList(),
                                             tr("Applications"), 3);
      this->appdbScrollArea->insertStretch();
      appdbClear->setEnabled(true);
      break;
...
void AppDBWidget::showXmlError(int id) {
  switch (id) {
    case 1:
      this->appdbHeader->addLabel(
          tr("Error: cannot read data from %1.").arg(APP_WEBSITE));
      break;
    case 2:
      this->appdbHeader->addLabel(
          tr("Error: wrong or broken XML data. Try again later."));
      break;
    case 3:
      this->appdbHeader->addLabel(
          tr("Error: wrong or broken AppDB XML version. This might be caused "
             "by an outdated application version."));
      break;
    case 4:
...
void AppDBWidget::createActions(void) {
  appdbOpen.reset(
      new QAction(CoreLib->loadIcon("wine"), tr("Open AppDB"), this));
  appdbOpen->setStatusTip(tr("Open Wine AppDB web site"));
  connect(appdbOpen.get(), SIGNAL(triggered()), this, SLOT(appdbOpen_Click()));

  appdbAppPage.reset(new QAction(CoreLib->loadIcon("document-preview"),
                                 tr("Open App page"), this));
  appdbAppPage->setStatusTip(
      tr("Open current application page at Wine AppDB web site"));
  appdbAppPage->setEnabled(false);
  connect(appdbAppPage.get(), SIGNAL(triggered()), this,
          SLOT(appdbAppPage_Click()));

  appdbClear.reset(new QAction(CoreLib->loadIcon("edit-clear-list"),
                               tr("Clear results"), this));
  appdbClear->setStatusTip(tr("Clear results"));
  appdbClear->setEnabled(false);
  connect(appdbClear.get(), SIGNAL(triggered()), this,
          SLOT(appdbClear_Click()));

  appdbClearSearch.reset(new QAction(CoreLib->loadIcon("edit-clear"),
                                     tr("Clear search field"), this));
  appdbClearSearch->setStatusTip(tr("Clear search field"));
  connect(appdbClearSearch.get(), SIGNAL(triggered()), this,
          SLOT(appdbClearSearch_Click()));

  appdbSearch.reset(
      new QAction(CoreLib->loadIcon("edit-find"), tr("Search in AppDB"), this));
  appdbSearch->setStatusTip(tr("Search in Wine AppDB"));
  connect(appdbSearch.get(), SIGNAL(triggered()), this,
          SLOT(appdbSearch_Click()));

  appdbCat.reset(new QAction(CoreLib->loadIcon("view-catalog"),
                             tr("AppDB categories list"), this));
...
void AppDBWidget::updateDataReadProgress(int bytesRead, int totalBytes) {
  this->appdbHeader->updateFirstLabelText(
      QString("Downloaded: %1 of %2 bytes").arg(bytesRead).arg(totalBytes));
  return;
}
...
void AppDBWidget::timer_timeout(void) {
  switch (this->action) {
    case 1:
      httpcore->getAppDBXMLPage(
          APPDB_HOSTNAME, APPDB_PORT,
          QString("/xmlexport/searchApp/%1/").arg(this->search));
      break;
    case 2:
      httpcore->getAppDBXMLPage(
          APPDB_HOSTNAME, APPDB_PORT,
          QString("/xmlexport/searchApp/%1/%2/").arg(this->search).arg(page));
      break;
    case 3:
      httpcore->getAppDBXMLPage(
          APPDB_HOSTNAME, APPDB_PORT,
          QString("/xmlexport/viewApp/%1/").arg(this->appid));
      break;
    case 4:
      httpcore->getAppDBXMLPage(APPDB_HOSTNAME, APPDB_PORT,
                                QString("/xmlexport/viewTest/%1/%2/%3/")
                                    .arg(this->appid)
                                    .arg(this->verid)
                                    .arg(this->testid));
      break;
    case 5:
      httpcore->getAppDBXMLPage(
          APPDB_HOSTNAME, APPDB_PORT,
          QString("/xmlexport/viewCategory/%1/").arg(this->catid));
      break;
  }
  timer->stop();
}
...
void AppDBWidget::stateChanged(int state) {
  switch (state) {
    ...
    case 3:
      this->appdbHeader->updateFirstLabelText(
          tr("Status: Sending request to the server."));
      break;
    case 4:
      this->appdbHeader->updateFirstLabelText(
          tr("Status: Reading server's response."));
      break;
    case 6:
      this->appdbHeader->updateFirstLabelText(
          tr("Status: Closing down connection."));
      this->setCursor(QCursor(Qt::ArrowCursor));
      break;
    case 0:
...
void AppDBWidget::appdbOpen_Click(void) {
  QDesktopServices::openUrl(
      QUrl(QString("http://%1/").arg("appdb.winehq.org"), QUrl::TolerantMode));
  return;
}
...
void AppDBWidget::appdbAppPage_Click(void) {
  if (this->verid == 0) {
    QDesktopServices::openUrl(
        QUrl(QString("http://%1/objectManager.php?sClass=application&iId=%2")
                 .arg("appdb.winehq.org")
                 .arg(this->appid),
             QUrl::TolerantMode));
  } else {
...
void AppDBWidget::appdbSearch_Click(void) {
  if (!searchField->text().isEmpty()) this->itemTrigged(1, searchField->text());
  return;
}
