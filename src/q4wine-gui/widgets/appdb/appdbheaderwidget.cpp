...
void AppDBHeaderWidget::addLink(QString text, bool enabled, short int action,
                                QString search, int value) {
  if (contentLayout.get()) {
    std::unique_ptr<LinkItemWidget> label(new LinkItemWidget(text, action));
    if (!enabled) label->setEnabled(enabled);
    ...
    connect(label.get(), SIGNAL(itemTrigged(short int, QString, int, int, int)),
            this, SIGNAL(itemTrigged(short int, QString, int, int, int)));
    contentLayout->addWidget(label.release());
  }
  return;
}

void AppDBHeaderWidget::setLayout(short int direction) { return; }
...
void AppDBHeaderWidget::createPagesList(short int count, short int current,
  ...
  if (start_page > 1) addLink("<", true, 2, search, start_page - 1);
  ...
  if (end_page < count) addLink(">", true, 2, search, end_page + 1);
...
void AppDBHeaderWidget::createCategoryList(
    const QList<WineAppDBCategory> category) {
  for (int i = 0; i < category.count(); i++) {
    ...
    if (i < category.count() - 1) this->addLabel(">");
  }
  return;
}
