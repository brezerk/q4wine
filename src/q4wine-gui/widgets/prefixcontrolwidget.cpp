...
PrefixControlWidget::PrefixControlWidget(QWidget *parent) : QWidget(parent) {
  ...
  connect(prefixTable.get(), SIGNAL(customContextMenuRequested(const QPoint &)),
          this, SLOT(customContextMenuRequested(const QPoint &)));
  connect(prefixTable.get(), SIGNAL(clicked(const QModelIndex &)), this,
...
void PrefixControlWidget::setDefaultFocus(QString prefixName) {
  if (prefixName.isEmpty()) return;

  int i = 0;
  while (1) {
    QModelIndex newIndex = model->index(i, 0);
    if (!newIndex.isValid()) break;
...
void PrefixControlWidget::createActions() {
  prefixAdd.reset(
      new QAction(CoreLib->loadIcon("document-new"), tr("Create new"), this));
  ...
  connect(prefixDelete.get(), SIGNAL(triggered()), this,
          SLOT(prefixDelete_Click()));
  prefixDelete->setEnabled(false);

  prefixSettings.reset(new QAction(CoreLib->loadIcon("document-edit"),
                                   tr("Edit prefix settings"), this));
  prefixSettings->setStatusTip(tr("Edit prefix settings"));
  connect(prefixSettings.get(), SIGNAL(triggered()), this,
          SLOT(prefixSettings_Click()));
  prefixSettings->setEnabled(false);

  prefixSetup.reset(new QAction(CoreLib->loadIcon("document-properties"),
                                tr("Set up prefix fake drive"), this));
  prefixSetup->setStatusTip(tr("Set up prefix fake drive and applications"));
  connect(prefixSetup.get(), SIGNAL(triggered()), this,
          SLOT(prefixSetup_Click()));
  prefixSetup->setEnabled(false);

  versionsSetup.reset(new QAction(CoreLib->loadIcon("configure"),
                                  tr("Configure wine versions"), this));
  versionsSetup->setStatusTip(tr("Configure wine versions"));
  connect(versionsSetup.get(), SIGNAL(triggered()), this,
          SLOT(versionsSetup_Click()));
...
void PrefixControlWidget::itemClicked(const QModelIndex &) {
  if (prefixTable->currentIndex().isValid()) {
    ...
    prefixSettings->setEnabled(true);
    prefixSetup->setEnabled(true);
    emit(prefixIndexChanged(
        model->index(prefixTable->currentIndex().row(), 0).data().toString()));
  } else {
    prefixDelete->setEnabled(false);
    prefixImport->setEnabled(false);
    prefixExport->setEnabled(false);
...
void PrefixControlWidget::updateTableModel() {
  model->setQuery(
      "SELECT name, arch, (SELECT name FROM versions WHERE id=version_id), "
      "path FROM prefix ORDER BY name");
...
void PrefixControlWidget::prefixDelete_Click() {
  if (!prefixTable->currentIndex().isValid()) return;

  if (prefixTable->currentIndex().row() < 0) return;

  QString prefixName =
      model->index(prefixTable->currentIndex().row(), 0, QModelIndex())
          .data()
          .toString();
  QString prefixPath =
      model->index(prefixTable->currentIndex().row(), 3, QModelIndex())
          .data()
          .toString();
  ...
  msgBox.setText(tr("Do you really wish to delete the prefix named \"%1\" and "
                    "all associated icons?")
                     .arg(prefixName));
...
void PrefixControlWidget::prefixImport_Click() {
  ...
  */
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Select file to import"), openpath,
      tr("Prefix archive images (*.tbz *.TBZ);;All files (*)"), 0, options);
  ...
  if (!fileName.isEmpty()) {
    ...
    if (dir.exists(targetDir)) {
      if (QMessageBox::warning(
              this, tr("Warning"),
              tr("Do you really wish to delete all prefix files at '%1'?")
                  .arg(targetDir),
        ...
        Process exportProcess(
            args, CoreLib->getWhichOut("rm"), QDir::homePath(),
            tr("Removing old fake drive at '%1'<br>This can take a while...")
                .arg(targetDir),
    ...
    Process exportProcess(
        args, CoreLib->getSetting("system", "tar").toString(), QDir::homePath(),
        tr("Importing prefix to '%1'<br>This can take a while...")
            .arg(targetDir),
        tr("Importing prefix"));
    exportProcess.exec();
  }
...
void PrefixControlWidget::updateDtabaseItems() { this->updateTableModel(); }
...
void PrefixControlWidget::prefixExport_Click() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Select file to export"), savepath,
      tr("Prefix archive images (*.tbz)"), 0, options);
  ...
  if (!fileName.isEmpty()) {
    ...
    // Creating process dialog
    Process exportProcess(
        args, CoreLib->getSetting("system", "tar").toString(), prefixPath,
        tr("Exporting %1 prefix.<br>This can take a while...").arg(prefixName),
        tr("Exporting prefix"));
    exportProcess.exec();
  }
