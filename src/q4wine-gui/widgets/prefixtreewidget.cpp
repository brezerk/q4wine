...
PrefixTreeWidget::PrefixTreeWidget(QWidget *parent) : QTreeWidget(parent) {
...
void PrefixTreeWidget::dirAdd_Click(void) {
  bool ok = false;
  QString dirname =
      QInputDialog::getText(this, tr("Enter new directory name"),
                            tr("Directory name:"), QLineEdit::Normal, "", &ok);
  std::unique_ptr<QTreeWidgetItem> treeItem(this->currentItem());
  ...
  if (ok && !dirname.isEmpty()) {
    while (db_dir.isExistsByName(this->prefixName, dirname)) {
      dirname = QInputDialog::getText(
          this, tr("Sorry. It seems that the directory already exists."),
          tr("Sorry. It seems that the directory already exists.<br>Please "
             "choose another name or cancel the operation."),
          QLineEdit::Normal, dirname, &ok);
      if ((!ok) || (dirname.isEmpty())) {
        treeItem.release();
        return;
      }
    ...
    if (treeItem->parent()) {
      std::unique_ptr<QTreeWidgetItem> prefixItem(
          new QTreeWidgetItem(treeItem->parent()));
      ...
      std::unique_ptr<QTreeWidgetItem> prefixItem(
          new QTreeWidgetItem(treeItem.get()));
      prefixItem->setText(0, dirname);
      if (dirname == "import") {
        prefixItem->setIcon(0, CoreLib->loadIcon("folder-cyan"));
      } else {
...
void PrefixTreeWidget::dirRename_Click(void) {
  ...
  if (!treeItem.get()) return;

  if (treeItem->parent()) {
    bool ok = false;
    QString newName = QInputDialog::getText(
        this, tr("Enter new name for directory"), tr("Directory name:"),
        QLineEdit::Normal, treeItem->text(0), &ok);
    ...
    if (ok && !newName.isEmpty()) {
      ...
      if (CoreLib->getSetting("Plugins", "enableMenuDesktop", false, true)
              .toBool()) {
        sys_menu.move_dir_info(treeItem->parent()->text(0), treeItem->text(0),
                               newName);
        sys_menu.writeXMLSystemMenu();
      }
#endif
...
void PrefixTreeWidget::dirDelete_Click(void) {
  ...
  if (treeItem->parent()) {
    if (QMessageBox::warning(this, tr("Q4Wine"),
                             tr("Do you really wish to delete the folder named "
                                "\"%1\" and all associated icons?\n")
                                 .arg(treeItem->text(0)),
                             QMessageBox::Yes,
...
void PrefixTreeWidget::getPrefixes() {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (CoreLib->getSetting("TreeWidget", "State", false, D_TREE_EXPAND)
            .toInt() == D_TREE_EXPAND) {
...
void PrefixTreeWidget::contextMenuEvent(QContextMenuEvent *event) {
  ...
  if (this->prefixMontPoint.isEmpty()) {
    ...
    entry.reset(
        new QAction(CoreLib->loadIcon("drive-optical"), tr("Browse..."), this));
  ...
  connect(menuRun.get(), SIGNAL(triggered(QAction *)), this,
          SLOT(menuRun_triggered(QAction *)));
  ...
  entry.reset(new QAction(CoreLib->loadIcon("document-new"),
                          tr("Create new prefix"), this));
  entry->setStatusTip(tr("Create new prefix"));
  connect(entry.get(), SIGNAL(triggered()), this,
          SLOT(menuCreatePrefix_Click()));
  menu->addAction(entry.release());

  entry.reset(new QAction(CoreLib->loadIcon("document-edit"),
                          tr("Configure prefix settings"), this));
  entry->setStatusTip(tr("Edit prefix settings"));
  connect(entry.get(), SIGNAL(triggered()), this,
          SLOT(menuConfigurePrefix_Click()));
  menu->addAction(entry.release());

  entry.reset(new QAction(CoreLib->loadIcon("configure"),
                          tr("Configure fake drive settings"), this));
  entry->setStatusTip(tr("Set up prefix fake drive and applications"));
  connect(entry.get(), SIGNAL(triggered()), this,
          SLOT(menuSetupPrefix_Click()));
  menu->addAction(entry.release());

  menu->addSeparator();

  entry.reset(
      new QAction(CoreLib->loadIcon("edit-delete"), tr("Delete prefix"), this));
  entry->setStatusTip(tr("Delete prefix"));
  connect(entry.get(), SIGNAL(triggered()), this,
          SLOT(menuDeletePrefix_Click()));
  menu->addAction(entry.release());
...
bool PrefixTreeWidget::dropMimeData(QTreeWidgetItem *parent, int index,
...
void PrefixTreeWidget::moveDesktopFile(QUrl url, QString prefixName,
                                       QString dirName) {
  QFile file(url.path());
  ...
  while (!in.atEnd()) {
    ...
    if (line.contains(QRegularExpression("^Exec=.*q4wine-cli.* "))) {
  ...
  while (db_icon.isExistsByName(prefixName, dirName, newName)) {
    newName = QInputDialog::getText(
        ...
        QLineEdit::Normal, icon, &ok);
    if (!ok) {
      return;
    }
  }
  if (!db_icon.updateIcon(newName, db_prefix.getId(prefixName),
                          db_dir.getId(dirName, prefixName),
                          db_prefix.getId(prefix), db_dir.getId(dir, prefix),
                          icon))
    return;
...
#ifdef DEBUG
  qDebug()
      << "Config key: advanced.openRunDialog="
...
void PrefixTreeWidget::menuDeletePrefix_Click(void) {
  if (this->prefixName == "Default") {
    QMessageBox::warning(this, tr("Error"),
                         tr("Sorry, you cannot delete the Default prefix."),
                         QMessageBox::Ok);
    return;
  }

  if (QMessageBox::warning(this, tr("Warning"),
                           tr("Do you really wish to delete the prefix named "
                              "\"%1\" and all associated icons?")
                               .arg(prefixName),
                           QMessageBox::Ok,
...
void PrefixTreeWidget::setDefaultFocus(QString prefixName, QString dirName) {
  if (prefixName.isEmpty()) {
    ...
    // skip update
    if (prefixName == this->prefixName) return;
...
void PrefixTreeWidget::expandTree() { this->expandAll(); }

void PrefixTreeWidget::collapseTree() { this->collapseAll(); }
