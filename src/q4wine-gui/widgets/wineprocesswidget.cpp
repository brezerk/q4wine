...
WineProcessWidget::WineProcessWidget(QWidget *parent) : QWidget(parent) {
  ...
  connect(procTable.get(), SIGNAL(customContextMenuRequested(const QPoint &)),
          this, SLOT(customContextMenuRequested(const QPoint &)));
  connect(procTable.get(), SIGNAL(clicked(const QModelIndex &)), this,
...
void WineProcessWidget::createActions() {
  procKillSelected.reset(
      new QAction(CoreLib->loadIcon("process-stop"), tr("Stop current"), this));
  procKillSelected->setStatusTip(tr("Send TERM signal to selected process"));
  procKillSelected->setEnabled(false);
  connect(procKillSelected.get(), SIGNAL(triggered()), this,
          SLOT(procKillSelected_Click()));

  procKillWine.reset(
      new QAction(CoreLib->loadIcon("system-shutdown"), tr("Stop wine"), this));
  procKillWine->setStatusTip(tr("Send TERM signal to main wine process"));
  procKillWine->setEnabled(false);
  connect(procKillWine.get(), SIGNAL(triggered()), this,
          SLOT(procKillWine_Click()));

  procRefresh.reset(
      new QAction(CoreLib->loadIcon("view-refresh"), tr("Refresh List"), this));
  procRefresh->setStatusTip(tr("Refresh process list"));
  connect(procRefresh.get(), SIGNAL(triggered()), this,
          SLOT(getWineProcesssInfo()));

  procRenice.reset(new QAction(tr("Renice"), this));
  procRenice->setStatusTip(tr("Set process priority"));
  procRenice->setEnabled(false);
  connect(procRenice.get(), SIGNAL(triggered()), this,
          SLOT(procRenice_Click()));
...
void WineProcessWidget::getWineProcesssInfo(void) {
  ...
  for (int i = 0; i < proclist.size(); ++i) {
    model->setData(model->index(i, 0, QModelIndex()), proclist.at(i).at(0),
                   Qt::DisplayRole);
    model->setData(model->index(i, 1, QModelIndex()), proclist.at(i).at(1),
                   Qt::DisplayRole);
    model->setData(model->index(i, 2, QModelIndex()), proclist.at(i).at(2),
                   Qt::DisplayRole);
    model->setData(model->index(i, 3, QModelIndex()), proclist.at(i).at(3),
                   Qt::DisplayRole);
  }
...
void WineProcessWidget::procKillSelected_Click(void) {
  if (QMessageBox::warning(
          this, tr("Warning"),
          tr("This action will send a KILL(-9) signal to selected "
             "processes<br><br>It is HIGH risk to damage wine normal "
             "state.<br><br>Do you really wish to proceed?"),
  ...
  foreach (index, indexes) {
    if (index.column() == 0)
      procList.append(
          model->index(index.row(), 0, QModelIndex()).data().toInt());
  }
  ...
  for (int i = 0; i < procList.count(); i++) {
    ...
    if (system(cmd.toLatin1().data()) == -1) {
      QMessageBox::warning(this, tr("Error"),
                           tr("Cannot run: %1").arg(cmd.toLatin1().data()),
                           QMessageBox::Ok);
#ifdef DEBUG
      ...
      emit(changeStatusText(
          tr("It seems that the process %1 has been killed successfully.")
              .arg(procList.at(i))));
#ifdef DEBUG
...
void WineProcessWidget::procKillWine_Click(void) {
  if (QMessageBox::warning(
          this, tr("Warning"),
          tr("This action will send a KILL(-9) signal to all wine processes of "
             "the selected prefixes<br><br>Do you really wish to proceed?"),
          QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
    return;
  ...
  foreach (index, indexes) {
    if (index.column() == 3) {
      ...
      QString pid =
          model->index(index.row(), 0, QModelIndex()).data().toString();
...
void WineProcessWidget::procRenice_Click(void) {
  ...
  int newNice = QInputDialog::getInt(
      this, tr("Select process priority"),
      tr("<p>Priority value can be in<br>the range from PRIO_MIN (-20)<br>to "
         "PRIO_MAX (20).</p><p>See \"man renice\" for details.</p>"),
      0, -20, 20, 1, &ok);
  if (!ok) return;
  ...
  for (int i = 0; i < procList.count(); i++) {
    if (CoreLib->reniceProcess(procList.at(i), newNice)) {
      emit(changeStatusText(tr("It seems that the process %1 has been reniced "
                               "to %2 successfully.")
                                .arg(procList.at(i))
                                .arg(newNice)));
    }
