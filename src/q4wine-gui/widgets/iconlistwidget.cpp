...
void IconListWidget::showContents(QString filterString) {
  ...
  QStringList iconsList = db_icon.getIconsList(this->prefixName, this->dirName,
                                               filterString, this->sort_order);
  ...
  for (int i = 0; i < iconsList.size(); ++i) {
    ...
    QString icon_path =
        db_icon.getPixmapIcon(this->prefixName, this->dirName, iconsList.at(i));
...
void IconListWidget::startDrag() {
  ...
  if (items.count() > 0) {
    ...
    for (int i = 0; i < items.count(); i++) {
      fileName = CoreLib->createDesktopFile(this->prefixName, this->dirName,
                                            items.at(i)->text());
...
void IconListWidget::startDrop(QList<QUrl> files) {
  ...
  for (int i = 0; i < files.count(); i++) {
    if (files.at(i).toLocalFile().contains(".exe", Qt::CaseInsensitive) ||
        files.at(i).toLocalFile().contains(".bat", Qt::CaseInsensitive) ||
        files.at(i).toLocalFile().contains(".com", Qt::CaseInsensitive)) {
      QString file = "", fileName = "";
      QStringList list1;
      file = files.at(i).toLocalFile();
      list1 = file.split("/");
      fileName = list1.last().left(list1.last().length() -
                                   list1.last().split(".").last().length() - 1);

      while (
          db_icon.isExistsByName(this->prefixName, this->dirName, fileName)) {
        bool ok = false;
        fileName = QInputDialog::getText(
            this, tr("Sorry. It seems that the icon already exists."),
            tr("Sorry. It seems that icon already exists.<br>Please choose "
               "another name or cancel the operation."),
      ...
      if (files.at(i).toLocalFile().contains(".bat", Qt::CaseInsensitive)) {
        ...
        db_icon.addIcon(file, "wineconsole", "", "", this->prefixName,
                        this->dirName, fileName, "", "", "", "",
                        localfile.left(localfile.length() -
                                       localfile.split("/").last().length()),
                        "", 0);
      } else {
        db_icon.addIcon(
            "", file, "", "", this->prefixName, this->dirName, fileName, "", "",
            "", "", file.left(file.length() - file.split("/").last().length()),
            "", 0);
      }
#ifndef _OS_DARWIN_
      if (CoreLib->getSetting("Plugins", "enableMenuDesktop", false, true)
              .toBool())
        CoreLib->createDesktopFile(this->prefixName, this->dirName, fileName,
                                   true);
#endif

    } else if (files.at(i).toLocalFile().contains(".iso",
                                                  Qt::CaseInsensitive) ||
               files.at(i).toLocalFile().contains(".mdf",
                                                  Qt::CaseInsensitive) ||
               files.at(i).toLocalFile().contains(".nrg",
                                                  Qt::CaseInsensitive)) {
      if (CoreLib->mountImage(files.at(i).toLocalFile(), this->prefixName)) {
        emit(changeStatusText(
            tr("%1 successfully mounted.").arg(files.at(i).toLocalFile())));
      } else {
        emit(changeStatusText(
            tr("Failed to mount %1.").arg(files.at(i).toLocalFile())));
      }
...
void IconListWidget::itemClicked(QListWidgetItem *item) {
  ...
  QHash<QString, QString> result =
      db_icon.getByName(this->prefixName, this->dirName, item->text());
  emit(iconItemClick(result.value("exec").split('/').last().split('\\').last(),
                     result.value("cmdargs"), result.value("desc"),
                     result.value("useconsole"), result.value("desktop")));
...
void IconListWidget::itemDoubleClicked(QListWidgetItem *item) {
  ...
  if (CoreLib->runIcon(this->prefixName, this->dirName, item->text())) {
    if (CoreLib->getSetting("app", "showTrareyIcon", false).toBool())
      if (CoreLib->getSetting("app", "minimizeToTrayAtAppStart", false)
              .toBool())
...
void IconListWidget::dragEnterEvent(QDragEnterEvent *event) {
  if (event->mimeData()->hasFormat("text/uri-list")) {
    ...
    for (int i = 0; i < list.count(); i++) {
      ...
      if (list.at(i).toLocalFile().contains(".exe", Qt::CaseInsensitive) ||
          list.at(i).toLocalFile().contains(".bat", Qt::CaseInsensitive) ||
          list.at(i).toLocalFile().contains(".com", Qt::CaseInsensitive) ||
          list.at(i).toLocalFile().contains(".iso", Qt::CaseInsensitive) ||
          list.at(i).toLocalFile().contains(".mdf", Qt::CaseInsensitive) ||
          list.at(i).toLocalFile().contains(".nrg", Qt::CaseInsensitive)) {
...
void IconListWidget::dragMoveEvent(QDragMoveEvent *event) {
  std::unique_ptr<IconListWidget> source(
      qobject_cast<IconListWidget *>(event->source()));
...
void IconListWidget::contextMenuEvent(QContextMenuEvent *event) {
  ...
  std::unique_ptr<QListWidgetItem> item(
      this->itemAt(event->pos().x(), event->pos().y()));
  if (item.get()) {
    isOnItemEvent = true;
  } else {
    isOnItemEvent = false;
  ...
  if (this->prefixMontPoint.isEmpty()) {
    ...
    std::unique_ptr<QMenu> submenuMount(new QMenu(
        tr("Mount [%1]").arg(CoreLib->getMountedImages(this->prefixMontPoint)),
        this));
    ...
    if (this->cdromDevices.count() > 0) {
      ...
      foreach (drive, this->cdromDevices) {
        entry.reset(
            new QAction(CoreLib->loadIcon("drive-optical"), drive, this));
        entry->setStatusTip(drive);
        submenuMount->addAction(entry.release());
      }
    } else {
      entry.reset(
          new QAction(CoreLib->loadIcon("drive-optical"), tr("[none]"), this));
    ...
    if (images.size() > 0) submenuMount->addSeparator();

    for (int i = 0; i < images.size(); ++i) {
      entry.reset(new QAction(CoreLib->loadIcon("media-optical"),
                              images.at(i).at(0), this));
      entry->setStatusTip(tr("Mount media image."));
      submenuMount->addAction(entry.release());
    }

    connect(submenuMount.get(), SIGNAL(triggered(QAction *)), this,
            SLOT(menuMount_triggered(QAction *)));
    ...
    entry.reset(
        new QAction(CoreLib->loadIcon("document-open"), tr("Browse..."), this));
    entry->setStatusTip(tr("Browse for media image."));
    submenuMount->addAction(entry.release());

    QStringList files =
        CoreLib->getSetting("", "recent_images", false).toStringList();

    if (files.size() > 0) submenuMount->addSeparator();

    for (int i = 0; i < files.size(); ++i) {
      entry.reset(new QAction(CoreLib->loadIcon("media-optical"),
                              files.at(i).split("/").last(), this));
    ...
    entry.reset(
        new QAction(tr("Unmount [%1]").arg(this->prefixMontPoint), this));
  ...
  if (isOnItemEvent) {
    ...
    entry->setStatusTip(
        tr("Paste selected icons from buffer to selected folder"));
    ...
    entry.reset(new QAction(CoreLib->loadIcon("document-open"),
                            tr("Open application directory"), this));
    entry->setStatusTip(
        tr("Open application directory in system file browser"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(xdgOpenIconDir_Click()));
    subMenu->addAction(entry.release());

    entry.reset(new QAction(CoreLib->loadIcon("drive-harddisk"),
                            tr("Open prefix directory"), this));
    entry->setStatusTip(tr("Open prefix directory in system file browser"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(xdgOpenPrefixDir_Click()));
    subMenu->addAction(entry.release());

    entry.reset(new QAction(CoreLib->loadIcon("media-optical"),
                            tr("Open mount point directory"), this));
    entry->setStatusTip(
        tr("Open mount point directory in system file browser"));
    if (this->prefixMontPoint.isEmpty()) entry->setEnabled(false);
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(xdgOpenMountDir_Click()));
    subMenu->addAction(entry.release());
    ...
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(winefileOpenIconDir_Click()));
    subMenu->addAction(entry.release());
    ...
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(winefileOpenPrefixDir_Click()));
    subMenu->addAction(entry.release());
    ...
    if (this->prefixMontPoint.isEmpty()) entry->setEnabled(false);
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(winefileOpenMountDir_Click()));
    subMenu->addAction(entry.release());
    ...
    entry.reset(new QAction(CoreLib->loadIcon("utilities-terminal"),
                            tr("Open console in application directory"), this));
    entry->setStatusTip(tr("Open system console in application directory"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(consoleToIconDir_Click()));
    subMenu->addAction(entry.release());

    entry.reset(new QAction(CoreLib->loadIcon("utilities-terminal"),
                            tr("Open console in prefix directory"), this));
    entry->setStatusTip(tr("Open system console in prefix directory"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(consoleToPrefixDir_Click()));
    subMenu->addAction(entry.release());

    entry.reset(new QAction(CoreLib->loadIcon("utilities-terminal"),
                            tr("Open console in mount point directory"), this));
    entry->setStatusTip(tr("Open system console in mount point directory"));
    if (this->prefixMontPoint.isEmpty()) entry->setEnabled(false);
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(consoleToMountDir_Click()));
    subMenu->addAction(entry.release());
    ...
    entry->setStatusTip(
        tr("Copy application directory path to system's clipboard"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(iconCopyWrkDir_Click()));
    subMenu->addAction(entry.release());

    entry.reset(new QAction(tr("Application path"), this));
    entry->setStatusTip(tr("Copy full application path to system's clipboard"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(iconCopyProgramPath_Click()));
    subMenu->addAction(entry.release());

    subMenu->addSeparator();

    entry.reset(
        new QAction(CoreLib->loadIcon("q4wine"), tr("q4wine-cli cmd"), this));
    entry->setStatusTip(tr("Copy q4wine-cli command for current application"));
    connect(entry.get(), SIGNAL(triggered()), this,
            SLOT(iconCopyQ4WineCmd_Click()));
    subMenu->addAction(entry.release());
    ...
    connect(menuRun.get(), SIGNAL(triggered(QAction *)), this,
            SLOT(menuRunRecent_triggered(QAction *)));
    ...
    entry.reset(new QAction(CoreLib->loadIcon("drive-optical"),
                            tr("Open mount point directory"), this));
    entry->setStatusTip(
        tr("Open mount point directory in system file browser"));
    if (this->prefixMontPoint.isEmpty()) entry->setEnabled(false);
...
void IconListWidget::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_Return) {
    if (this->currentItem()) this->itemDoubleClicked(this->currentItem());
  } else if (event->key() == Qt::Key_Delete) {
    if (this->currentItem()) this->iconDelete_Click();
  } else if (event->key() == Qt::Key_F2) {
    if (this->currentItem()) this->iconRename_Click();
...
void IconListWidget::iconRename_Click(void) {
  ...
  QString newName =
      QInputDialog::getText(this, tr("Enter new icon name"), tr("Icon name:"),
                            QLineEdit::Normal, iconItem->text(), &ok);
  if (ok && !newName.isEmpty()) {
    while (db_icon.isExistsByName(this->prefixName, this->dirName, newName)) {
      newName = QInputDialog::getText(
          this, tr("Sorry. It seems that the icon already exists."),
          tr("Sorry. It seems that the icon already exists.<br>Please choose "
             "another name or cancel the operation."),
    ...
    db_icon.renameIcon(iconItem->text(), this->prefixName, this->dirName,
                       newName);
#ifndef _OS_DARWIN_
    if (CoreLib->getSetting("Plugins", "enableMenuDesktop", false, true)
            .toBool()) {
      CoreLib->deleteDesktopFile(this->prefixName, this->dirName,
                                 iconItem->text());
      CoreLib->createDesktopFile(this->prefixName, this->dirName, newName,
                                 true);
      sys_menu.writeXMLSystemMenu();
    }
#endif
...
void IconListWidget::iconDelete_Click(void) {
  ...
  if (icoList.count() <= 0) return;

  if (QMessageBox::warning(
          this, tr("Delete Icon"),
          tr("Do you wish to delete all of the selected icons?"),
...
void IconListWidget::iconRun_Click(void) {
  ...
  if (iconItem.get()) this->itemDoubleClicked(iconItem.get());
...
void IconListWidget::iconCut_Click(void) {
  ...
  // FIXME: It pice works fine, but we need to use pixmaps for grayscale, not
  ...
  //		icoList2.at(i)->icon()->addPixmap(QPixmap.alphaChannel (), 0,
  //0);
  //	}
    ...
    for (int i = 0; i < iconBuffer.names.count(); i++) {
      ...
      while (db_icon.isExistsByName(this->prefixName, this->dirName, newName)) {
        newName = QInputDialog::getText(
            ...
            tr("Sorry. It seems that the icon already exists.<br>Please choose "
               "another name or cancel the operation."),
            QLineEdit::Normal, iconBuffer.names.at(i), &ok);
        if (!ok) {
          return;
        }
      }
      ...
      if (!iconBuffer.move) {
        if (!db_icon.copyIcon(iconBuffer.names.at(i), iconBuffer.prefix_name,
                              iconBuffer.dir_name, newName, this->prefixName,
                              this->dirName))
          return;
#ifndef _OS_DARWIN_
        if (CoreLib->getSetting("Plugins", "enableMenuDesktop", false, true)
        ...
        if (!db_icon.updateIcon(
                newName, db_prefix.getId(this->prefixName),
                db_dir.getId(this->dirName, this->prefixName),
                db_prefix.getId(iconBuffer.prefix_name),
                db_dir.getId(iconBuffer.dir_name, iconBuffer.prefix_name),
                iconBuffer.names.at(i)))
          return;
#ifndef _OS_DARWIN_
        if (CoreLib->getSetting("Plugins", "enableMenuDesktop", false, true)
                .toBool()) {
          CoreLib->deleteDesktopFile(iconBuffer.prefix_name,
                                     iconBuffer.dir_name,
                                     iconBuffer.names.at(i));
          CoreLib->createDesktopFile(this->prefixName, this->dirName, newName,
                                     true);
  ...
  if (this->dirName == "import") {
    ...
    if (list.count() > 1) {
      for (int i = 1; i < list.count(); i++) {
        ...
        if (i < list.count() - 1) iconText.append(" - ");
      }
    }
  }
...
void IconListWidget::iconCopyWrkDir_Click() {
  ...
  QHash<QString, QString> info =
      db_icon.getByName(this->prefixName, this->dirName, iconItem->text());
...
void IconListWidget::iconCopyQ4WineCmd_Click() {
  ...
  QString run_string =
      QString("%1/bin/q4wine-cli ").arg(QString::fromUtf8(APP_PREF));
  run_string.append(" -p \"");
...
void IconListWidget::runProgramRequest(QString message) {
  ...
  if (message.isEmpty()) {
    ...
    QString wrkDir;
    wrkDir = message.left(message.length() -
                          QStringList(message.split("/")).last().length());
...
void IconListWidget::menuRunRecent_triggered(QAction *action) {
...
#ifdef DEBUG
  qDebug()
      << "[ii] Config key: advanced.openRunDialog="
      << CoreLib->getSetting("advanced", "openRunDialog", false, 0).toString();
#endif
  ...
  if (CoreLib->getSetting("advanced", "openRunDialog", false, 0).toInt() == 0) {
    ...
    Run run;
    run.prepare(this->prefixName, result.at(0), result.at(1), result.at(2),
                result.at(3), result.at(4), result.at(5), result.at(6),
                result.at(7).toInt(), action->statusTip(), result.at(8));
    run.exec();
    return;
  }
  return;
...
void IconListWidget::menuMount_triggered(QAction *action) {
  /*
   * This slot process menuDirMountImages and menuIconMountImages triggered
   * signal
   */

  if (action->text() == tr("[none]")) {
    QMessageBox::warning(this, tr("Error"),
                         tr("No device drive specified in prefix settings."),
                         QMessageBox::Ok);
    return;
...
#ifdef _OS_LINUX_
    fileFilter =
        tr("Disc image files (*.iso *.ISO *.nrg *.NRG *.img *.IMG *.bin *.BIN "
           "*.mdf *.MDF);;All files (*)");
#else
    ...
    QString fileName =
        QFileDialog::getOpenFileName(this, tr("Open Disc Image Files"),
                                     QDir::homePath(), fileFilter, 0, options);
...
void IconListWidget::xdgOpenPrefixDir_Click(void) {
  QDesktopServices::openUrl(
      QUrl(QString("file://%1").arg(db_prefix.getPath(this->prefixName)),
           QUrl::TolerantMode));
  return;
}
...
void IconListWidget::xdgOpenMountDir_Click(void) {
  QDesktopServices::openUrl(QUrl(
      QString("file://%1").arg(this->prefixMontPoint), QUrl::TolerantMode));
  return;
}
...
void IconListWidget::xdgOpenIconDir_Click(void) {
  ...
  if (result.isEmpty()) {
    ...
    QDesktopServices::openUrl(
        QUrl(QString("file://%1").arg(result), QUrl::TolerantMode));
  }
...
void IconListWidget::consoleToMountDir_Click(void) {
  if (this->prefixMontPoint.isEmpty()) return;
