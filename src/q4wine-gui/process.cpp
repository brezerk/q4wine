...
Process::Process(QStringList args, QString exec, QString dir, QString info,
                 QString caption, bool showErr, QStringList env,
  ...
  connect(myProcess.get(), SIGNAL(finished(int, QProcess::ExitStatus)), this,
          SLOT(slotFinished(int, QProcess::ExitStatus)));
  connect(myProcess.get(), SIGNAL(error(QProcess::ProcessError)), this,
...
void Process::slotError(QProcess::ProcessError err) {
  if (myProcess->exitCode() != 0) {
    ...
    /*
            ...
            if (!codec){
                    stdErr<<"[ee] Cannot setup codec for
       \""<<lang<<"\""<<Qt::endl; stdErr<<"[ee] Aborting current
       operation!"<<Qt::endl; reject(); return;
            }
            QString string =
       codec->toUnicode(myProcess->readAllStandardError());
    */
    ...
    if (!string.isEmpty()) {
      QMessageBox::warning(
          this, tr("Error"),
          tr("It seems that the process failed.<br><br>Error log:<br>%1")
              .arg(string));
    } else {
      switch (err) {
        case 0:
          QMessageBox::warning(
              this, tr("Error"),
              tr("Process: The process failed to start. Either the invoked "
                 "program is missing, or you may have insufficient permissions "
                 "to invoke the program."));
          break;
        case 1:
          QMessageBox::warning(this, tr("Error"),
                               tr("Process: The process crashed some time "
                                  "after starting successfully."));
          break;
        case 2:
          QMessageBox::warning(
              this, tr("Error"),
              tr("Process: The last waitFor...() function timed out."));
          break;
        case 3:
          QMessageBox::warning(
              this, tr("Error"),
              tr("Process: An error occurred when attempting to read from the "
                 "process. For example, the process may not be running."));
          break;
        case 4:
          QMessageBox::warning(
              this, tr("Error"),
              tr("Process: An error occurred when attempting to write to the "
                 "process. For example, the process may not be running, or it "
                 "may have closed its input channel."));
          break;
        case 5:
          QMessageBox::warning(this, tr("Error"),
                               tr("Process: An unknown error occurred. This is "
                                  "the default return value of error()."));
          break;
      }
      reject();
    }
    ...
    switch (err) {
      ...
      case 1:
        QMessageBox::warning(this, tr("Error"),
                             tr("Process: The process crashed some time after "
...
void Process::slotFinished(int, QProcess::ExitStatus exitc) {
   ...
   * So the beast way is to inform user about troubles is to show to him any
   * STDERR messages.
   */
...
#ifdef DEBUG
  qDebug() << "[ii] Process::slotFinished exitstatus:" << exitc
           << "exitcode:" << myProcess->exitCode();
  qDebug() << "[ii] Process::slotFinished STDERR:"
           << myProcess->readAllStandardError();
  qDebug() << "[ii] Process::slotFinished STDOUT:"
    ...
    if ((exitc == 0) && (myProcess->exitCode() == 0)) {
      if (showErr) {
        QMessageBox::warning(this, tr("Output"),
                             tr("It seems that the process exited "
                                "normally.<br><br>STDERR log:<br>%1")
                                 .arg(string));
      }
      accept();
      return;
      ...
      QMessageBox::warning(
          this, tr("Output"),
          tr("It seems that the process crashed.<br><br>STDERR log:<br>%1")
              .arg(string));
      reject();
      return;
    }
