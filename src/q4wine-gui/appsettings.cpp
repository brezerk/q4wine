...
AppSettings::AppSettings(QWidget *parent, Qt::WindowFlags f)
    : QDialog(parent, f) {
  ...
  connect(cmdVersionManager, SIGNAL(clicked()), this,
          SLOT(cmdVersionManager_Click()));
  ...
  connect(cbShowTray, SIGNAL(stateChanged(int)), this,
          SLOT(cbShowTray_stateChanged(int)));
  ...
  if (appIcon == "q4wine-ambiance") {
    comboAppIcons->setCurrentIndex(
        comboAppIcons->findText(tr("Ambient Light")));
  ...
  if (settings.value("lang").toString().isEmpty()) {
    ...
    comboLangs->setCurrentIndex(
        comboLangs->findText(QLocale(lName).nativeLanguageName()));
  }
  ...
  if (!settings.value("mount_image_string").toString().isEmpty())
    txtMountImageString->setText(
        settings.value("mount_image_string").toString());
  ...
  settings.endGroup();
  connect(cmdDesktopMenu_Regen, SIGNAL(clicked()), this,
          SLOT(cmdDesktopMenu_Regen_Click()));
  connect(cmdDesktopMenu_Remove, SIGNAL(clicked()), this,
          SLOT(cmdDesktopMenu_Remove_Click()));
#else
  ...
  this->txtWineDesktopDoc->setText(
      settings.value("WineDesktopDoc", "").toString());
  this->txtWineDesktopMus->setText(
      settings.value("WineDesktopMus", "").toString());
  this->txtWineDesktopPic->setText(
      settings.value("WineDesktopPic", "").toString());
  this->txtWineDesktopVid->setText(
      settings.value("WineDesktopVid", "").toString());
  settings.endGroup();
...
void AppSettings::cmdOk_Click() {
  ...
  if (!checkEntry(txtGuiSudoBin->text(), "gui_sudo")) return;
