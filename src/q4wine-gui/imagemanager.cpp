...
ImageManager::ImageManager(QWidget *parent, Qt::WindowFlags f)
    : QDialog(parent, f) {
  // setModal(true);
  setupUi(this);
  ...
  connect(tableImage, SIGNAL(customContextMenuRequested(const QPoint &)), this,
          SLOT(tableImage_showContextMenu(const QPoint &)));
  connect(tableImage, SIGNAL(clicked(const QModelIndex &)), this,
          SLOT(update_lblPathInfo(const QModelIndex &)));
...
void ImageManager::update_lblPathInfo(const QModelIndex) {
  if (tableImage->currentRow() < 0) return;
  lblPathInfo->setText(
      tr("File path: %1")
          .arg(db_image.getPath(
              tableImage->item(tableImage->currentRow(), 0)->text())));
...
void ImageManager::getCDImages(void) {
  ...
  for (int i = 0; i < result.size(); ++i) {
    ...
    if (tableImage->item(curRows - 1, 0)) {
      ...
      if (!file.exists()) {
        tableImage->item(curRows - 1, 0)
            ->setIcon(CoreLib->loadIcon("dialog-warning"));
      } else {
        tableImage->item(curRows - 1, 0)->setIcon(QIcon());
      }
    } else {
      std::unique_ptr<QTableWidgetItem> newItem(
          new QTableWidgetItem(result.at(i).at(0)));
      if (!file.exists()) {
  ...
  if (numRows > curRows)
    for (int i = curRows; i <= numRows; i++) tableImage->removeRow(curRows);
...
void ImageManager::loadThemeIcons() {
  ...
  actionAdd.reset(managerToolBar->addAction(CoreLib->loadIcon("document-new"),
                                            tr("Add image")));
  connect(actionAdd.get(), SIGNAL(triggered()), this, SLOT(actionAddImage()));

  actionRename.reset(managerToolBar->addAction(
      CoreLib->loadIcon("document-properties"), tr("Rename image")));
  connect(actionRename.get(), SIGNAL(triggered()), this,
          SLOT(actionRenameImage()));

  managerToolBar->addSeparator();

  actionRemove.reset(managerToolBar->addAction(CoreLib->loadIcon("edit-delete"),
                                               tr("Remove image")));
  connect(actionRemove.get(), SIGNAL(triggered()), this,
          SLOT(actionRemoveImage()));

  managerToolBar->addSeparator();

  actionRefresh.reset(managerToolBar->addAction(
      CoreLib->loadIcon("view-refresh"), tr("Refresh image list")));
  connect(actionRefresh.get(), SIGNAL(triggered()), this,
          SLOT(actionRefreshImageList()));
...
void ImageManager::actionAddImage() {
  ...
  fileName =
      QFileDialog::getOpenFileName(this, tr("Open ISO image file"),
  ...
  if (!fileName.isEmpty()) {
    ...
    while (db_image.isExistsByName(newName)) {
      newName = QInputDialog::getText(
          this, tr("Sorry. It seems that the CD image already exists."),
          tr("Sorry. It seems that the CD image file already exists.<br>Please "
             "rename it, or cancel add image operation."),
          QLineEdit::Normal, fileName.split("/").last(), &ok);
      if (!ok) {
        return;
      }
    }
    if (!db_image.addImage(newName, fileName)) return;
...
void ImageManager::actionRenameImage() {
  if (tableImage->currentRow() == -1) return;

  bool ok = false;
  QString newName = QInputDialog::getText(
      this, tr("Enter new name"), tr("Enter new name:"), QLineEdit::Normal,
      tableImage->item(tableImage->currentRow(), 0)->text(), &ok);
  while (db_image.isExistsByName(newName) or (newName.isEmpty())) {
    ...
    if (newName.isEmpty()) {
      newName = QInputDialog::getText(
          this, tr("Sorry. CD image name cannot be empty."),
          tr("Sorry. CD image name cannot be empty.<br>Please enter new name, "
             "or cancel rename image operation."),
          QLineEdit::Normal, newName, &ok);
    } else {
      newName = QInputDialog::getText(
          this, tr("Sorry. It seems that the CD image already exists."),
          tr("Sorry. It seems that the CD image file already exists.<br>Please "
             "rename it, or cancel rename image operation."),
          QLineEdit::Normal, newName, &ok);
    }
  }
  if (!db_image.renameImage(
          newName, tableImage->item(tableImage->currentRow(), 0)->text()))
    return;
  actionRefreshImageList();
  return;
}
