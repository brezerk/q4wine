...
void IconSettings::cmdGetIcon_Click(){
	...
	fileName = QFileDialog::getOpenFileName(this, tr("Open image file"), searchPath, tr("Image files (*.png *.jpg *.gif *.bmp);;Win32 Executable (*.exe *.dll)") );

	if(!fileName.isEmpty()){
		
		if ((fileName.right(3)!="exe") && (fileName.right(3)!="dll")){
